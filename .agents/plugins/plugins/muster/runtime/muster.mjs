...
async function registerPlugin(execFile6, dryRun, expectedVersion) {
  ...
  try {
    const marketplaces = await runJson(execFile6, ["plugin", "marketplace", "list", "--json"]);
    if (!marketplaces?.marketplaces?.some((item) => item.name === "muster")) {
      await run(execFile6, ["plugin", "marketplace", "add", CODEX_MARKETPLACE]);
      marketplaceAdded = true;
    }
    const plugins = await runJson(execFile6, ["plugin", "list", "--available", "--json"]);
    const records2 = [...plugins?.installed || [], ...plugins?.available || []];
    const installed = records2.find((item) => (item.pluginId === CODEX_PLUGIN || item.name === "muster" && item.marketplaceName === "muster") && item.installed);
...
async function runCodexInstall({ scope = "project", dryRun = false, cwd = process.cwd(), home = homedir7(), repoRoot, execFile: execFile6 = execFileDefault2 } = {}) {
  ...
  const packagePlugin = await readJson2(join14(root, ".agents", "plugins", "plugins", "muster", ".codex-plugin", "plugin.json")) || await readJson2(join14(root, ".codex-plugin", "plugin.json"));
  ...
  let actions = [];
  try {
    actions = present ? await registerPlugin(execFile6, dryRun, expectedPluginVersion) : [];
  } catch (error) {
