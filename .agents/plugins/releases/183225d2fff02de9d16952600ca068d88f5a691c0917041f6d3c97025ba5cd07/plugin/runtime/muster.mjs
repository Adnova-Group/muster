...
async function readInstalledCowork(home, opts = {}) {
  ...
  return {
    ...
    skills: [],
    agents: [],
    mcpServers: [.../* @__PURE__ */ new Set([...discovered, ...declaredConnectors])],
...
function resolveCapabilities(catalog, installed, home = homedir2()) {
  ...
  for (const role of ROLES) {
    ...
    for (const e of forRole) {
      if (e.kind === "external" && e.recommended && !isInstalled(e, installed) && e.rank > chosenRank && (!cowork || providerType(e) === "mcp")) {
...
import { readFile as readFile14, writeFile as writeFile5, mkdir as mkdir8 } from "node:fs/promises";
...
function semverCompare(a, b) {
  const parse7 = (v) => String(v).split(".").map(Number);
  const [aMaj, aMin, aPat = 0] = parse7(a);
  const [bMaj, bMin, bPat = 0] = parse7(b);
...
async function runDoctor({ root, home, exec } = {}) {
  ...
  }
  {
    ...
    let readError;
    try {
      raw = await readFile5(manifestPath, "utf8");
    } catch (error) {
      readError = error;
    }
    if (readError) {
...
import { mkdir as mkdir3, readdir as readdir8, lstat as lstat2, realpath, open, rename, unlink } from "node:fs/promises";
import { constants } from "node:fs";
import { join as join11, dirname as dirname2, sep, resolve, relative, isAbsolute, basename } from "node:path";
...
async function ensureOutputDirectory(root, directory) {
  ...
  for (const part of parts) {
    ...
    if (!info) {
      await mkdir3(cursor, { mode: 493 });
      info = await lstat2(cursor);
    }
    if (info.isSymbolicLink() || !info.isDirectory())
      throw new Error(`unsafe output path: ${cursor} is a symlink or special component`);
    const canonical2 = await realpath(cursor);
    if (!contained(root, canonical2)) throw new Error("unsafe output path: canonical ancestry escapes repoRoot");
  }
}
...
async function atomicPublish(root, relativePath, content) {
  ...
  await ensureOutputDirectory(root, dirname2(target));
  await readOutputRegular(target);
  const bytes = Buffer.isBuffer(content) ? content : Buffer.from(content);
  const temp = join11(dirname2(target), `.${basename(target)}.${process.pid}.${randomBytes(12).toString("hex")}.muster-tmp-`);
  let handle;
  try {
    handle = await open(temp, constants.O_CREAT | constants.O_EXCL | constants.O_WRONLY | NOFOLLOW, 384);
...
async function runVendor({ home = homedir5(), repoRoot = process.cwd(), manifest } = {}) {
  ...
  for (const source of manifest.sources) {
    ...
    for (const item of source.items) {
      ...
      let text;
      try {
        text = await readSourceRegular(root, item.from);
      } catch (error2) {
        warnings.push(`${source.id}: unsafe source path for ${item.from}: ${error2.message}`);
        continue;
      }
      if (text === null) {
      ...
      const abs = join11(outputRoot, path);
  ...
  if (builtinEntries.length > 0)
    await atomicPublish(outputRoot, "catalog/builtins.generated.yaml", YAML_BANNER + (0, import_yaml5.stringify)(builtinEntries, { lineWidth: 0 }));
  if (agentEntries.length > 0)
    await atomicPublish(outputRoot, "catalog/agents.generated.yaml", YAML_BANNER + (0, import_yaml5.stringify)(agentEntries, { lineWidth: 0 }));
...
// src/install.js
import { readFile as readFile7, lstat as lstat3, open as open2, rename as rename2, unlink as unlink2 } from "node:fs/promises";
import { constants as constants2 } from "node:fs";
import { join as join13, basename as basename2, dirname as dirname4 } from "node:path";
...
var NOFOLLOW2 = constants2.O_NOFOLLOW || 0;
var LEGACY_STYLE_DIGESTS = /* @__PURE__ */ new Set([
...
async function safeLegacyAncestry(home) {
  for (const path of [home, join13(home, ".claude"), join13(home, ".claude", "output-styles")]) {
    const stat2 = await statIfExists(path);
    if (stat2 === null) return "absent";
    if (stat2.isSymbolicLink() || !stat2.isDirectory()) return "unsafe";
  }
  return "safe";
}
async function readRegularNoFollow(path) {
  ...
  let handle;
  try {
    handle = await open2(path, constants2.O_RDONLY | NOFOLLOW2);
...
async function verifyUnchangedRegular(path, expected) {
  ...
  return current.kind === "regular" && sameFileIdentity(expected.stat, current.stat) && Buffer.compare(expected.bytes, current.bytes) === 0;
}
async function atomicReplaceRegular(path, bytes, expectedDest) {
  const parent = dirname4(path);
  const temp = join13(parent, `.${basename2(path)}.${process.pid}.${randomBytes2(12).toString("hex")}.muster-tmp-`);
  let handle;
  try {
    handle = await open2(temp, constants2.O_CREAT | constants2.O_EXCL | constants2.O_WRONLY | NOFOLLOW2, 384);
    await handle.writeFile(bytes);
    await handle.sync();
    await handle.close();
    handle = null;
...
async function runUninstall({ home = homedir6() } = {}) {
  ...
  if (existing.kind === "absent") {
    ...
    if (backup.kind === "unsafe") {
      ...
      if (backupNow.kind !== "regular" || !sameFileIdentity(backup.stat, backupNow.stat) || Buffer.compare(backup.bytes, backupNow.bytes) !== 0)
...
import { closeSync, constants as fsConstants2, linkSync, openSync, readFileSync as readFileSync2, readlinkSync, renameSync, rmSync } from "node:fs";
import { cp, lstat as lstat5, mkdir as mkdir6, mkdtemp, open as open4, readFile as readFile9, readdir as readdir9, rename as rename4, rm, writeFile as writeFile4 } from "node:fs/promises";
import { isAbsolute as isAbsolute2, join as join15, relative as relative2, resolve as resolve2 } from "node:path";
...
async function processStartIdentity(pid = process.pid) {
  ...
  try {
    const stat2 = await readFile8(`/proc/${pid}/stat`, "utf8");
...
var CURRENT_RELEASE_FORMAT = 2;
var SUPPORTED_RELEASE_FORMATS = /* @__PURE__ */ new Set([1, CURRENT_RELEASE_FORMAT]);
...
async function readRegular(path, label, maxBytes = 32 * 1024 * 1024) {
  ...
  try {
    ...
    handle = await open4(path, fsConstants2.O_RDONLY | (fsConstants2.O_NOFOLLOW || 0));
    const stat2 = await handle.stat();
...
function contained2(base, target, label) {
  const rel = relative2(resolve2(base), resolve2(target));
  if (!rel || rel === ".") return;
...
async function createCodexBootstrapMetadata(bootstrapRoot) {
...
async function createCodexReleaseMetadataForFormat(releaseRoot, packageVersion, format) {
  ...
  if (format === 1) {
    if (!tree.dirs.includes("plugin") || !tree.dirs.includes("profiles") || !files.some((file) => file.path.startsWith("plugin/")) || !files.some((file) => file.path.startsWith("profiles/"))) {
    ...
    if (!tree.dirs.includes("plugin") || !tree.dirs.includes("plugin/agents") || !files.some((file) => file.path.startsWith("plugin/agents/") && file.path.endsWith(".toml")) || !files.some((file) => file.path === "plugin/runtime/muster.mjs")) {
...
function releaseProfilesRoot(releaseRoot, metadata) {
  return metadata.format === CURRENT_RELEASE_FORMAT ? join15(releaseRoot, "plugin", "agents") : join15(releaseRoot, "profiles");
}
function releasePluginRoot(releaseRoot) {
  return join15(releaseRoot, "plugin");
}
...
var RELEASE_PLUGIN_PATH = /^\.\/\.agents\/plugins\/releases\/([a-f0-9]{64})\/plugin$/;
...
var leaseRegistryLockPath = (repoRoot) => join15(repoRoot, ".agents", "plugins", ".lease-registry.lock");
...
async function renewLease(path, token, record, replaceLease = rename4) {
...
async function registerGenerationLease(repoRoot, generation, leaseOptions = {}) {
  const key = `${resolve2(repoRoot)}\0${generation}`;
...
async function createGenerationLease(repoRoot, generation, leaseOptions, key) {
  ...
  const replaceLease = leaseOptions.replaceLease || rename4;
  const beforeLeaseCleanup = leaseOptions.beforeLeaseCleanup;
  const identity = await processStartIdentity();
  const processNamespace = typeof leaseOptions.processNamespace === "string" && leaseOptions.processNamespace ? leaseOptions.processNamespace : localProcessNamespace();
  const base = { format: RELEASE_FORMAT, pid: process.pid, processIdentity: identity, processNamespace, processStartedAt: Math.floor(Date.now() - process.uptime() * 1e3), generation, token };
...
async function resolveCodexReleaseWithOptions(repoRoot, { retries = 4, readSelections, lease } = {}) {
  ...
  if (pointer?.name !== "muster" || pluginPath !== STABLE_BOOTSTRAP_PATH && !advertisedGeneration || bootstrap?.format !== RELEASE_FORMAT || !GENERATION.test(bootstrap?.initialGeneration || "") || !GENERATION.test(bootstrap?.digest || "")) {
...
// src/codex-thread-limits.js
import { join as join16, resolve as resolve3 } from "node:path";
var REQUIRED_CODEX_THREAD_LIMITS = Object.freeze({ max_threads: 12, max_depth: 2 });
function discoverCodexGlobalHomes({ cwd, home, codexHome: codexHome2, globalHomes } = {}) {
  const candidates = globalHomes?.length ? globalHomes : [codexHome2 || join16(home, ".codex")];
  ...
  for (const candidate of candidates) {
    const path = resolve3(candidate), key = /^\/mnt\/[a-z]\//i.test(path) ? path.toLowerCase() : path;
    if (!seen.has(key)) {
      seen.add(key);
      result.push(path);
    }
...
var render = (state) => state.lines.join(state.newline) + (state.finalNewline || state.lines.length ? state.newline : "");
function ensureCodexThreadLimits(text) {
  const state = parse4(text), before = {}, installed = {}, sectionCreated = state.start < 0;
...
function restoreCodexThreadLimits(text, record) {
  const state = parse4(text);
  for (const key of Object.keys(REQUIRED_CODEX_THREAD_LIMITS)) {
    const current = state.values[key];
  ...
  if (record.sectionCreated) {
    const reparsed = parse4(render(state));
...
var threadLimitManifestPath = (globalHome) => join16(globalHome, "muster", "thread-limits.json");
...
var SCOPE_LOCK_MAX_STALE_MS = 15 * 6e4;
var codexHome = (home) => process.env.CODEX_HOME || join17(home, ".codex");
var agentsDir = (scope, cwd, home) => scope === "user" ? join17(codexHome(home), "agents") : join17(cwd, ".codex", "agents");
var configDir = (scope, cwd, home) => scope === "user" ? codexHome(home) : join17(cwd, ".codex");
var scopeRegistryPath = (home) => join17(codexHome(home), "muster", "install-scopes.json");
var scopeRegistryLockPath = (home) => `${scopeRegistryPath(home)}.lock`;
async function ordinaryDirectoryPath(path, { create = false } = {}) {
  const absolute = resolve4(path), root = parse5(absolute).root;
  let current = root;
  for (const part of relative3(root, absolute).split(sep2).filter(Boolean)) {
    current = join17(current, part);
    let stat2;
    try {
      stat2 = await lstat6(current);
    } catch (error) {
      if (error.code !== "ENOENT") throw error;
...
async function regularFileState(path) {
  await ordinaryDirectoryPath(dirname6(path));
  let stat2;
  try {
    stat2 = await lstat6(path);
  } catch (error) {
    if (error.code === "ENOENT") return null;
    throw error;
  }
  if (stat2.isSymbolicLink() || !stat2.isFile()) throw new Error(`Codex configuration target must be a regular file: ${path}`);
  return stat2;
}
async function safeExists(path) {
  return Boolean(await regularFileState(path));
}
...
var readJson2 = async (path) => {
  try {
    return JSON.parse(await readSafe(path, "utf8"));
  } catch (error) {
...
async function readScopeRegistry(home) {
  ...
  const registry = await readJson2(path);
  ...
  for (const entry of registry.entries) {
    if (!entry || !["project", "user"].includes(entry.scope) || typeof entry.configDir !== "string" || !isAbsolute3(entry.configDir)) {
    ...
    seen.add(key);
    entries.push({ scope: entry.scope, configDir: entry.configDir });
  }
...
async function scopeEntry(scope, cwd, home) {
  ...
  try {
    return { scope, configDir: await realpath2(dir) };
  } catch (error) {
    if (error.code === "ENOENT") return { scope, configDir: resolve4(dir) };
    throw error;
  }
}
...
function parseScopeLock(text, path) {
  ...
  if (lock?.format !== 1 || lock.owner !== "muster" || !Number.isSafeInteger(lock.pid) || lock.pid < 1 || typeof lock.token !== "string" || !lock.token || !Number.isFinite(lock.createdAt) || lock.createdAt < 0 || Object.hasOwn(lock, "processIdentity") && lock.processIdentity !== null && typeof lock.processIdentity !== "string") {
    throw new Error(`Codex managed-scope lock is invalid: ${path}`);
  }
  return lock;
}
...
async function atomicWriteSafe(path, content) {
  ...
  const temporary = join17(parent, `.${basename3(path)}.muster-${process.pid}-${Date.now()}-${Math.random().toString(16).slice(2)}.tmp`);
  let handle;
  try {
...
async function removeSafe(path) {
  ...
  if (stat2) await unlink4(path);
}
async function prepareThreadLimitInstalls({ cwd, home, globalHomes }) {
  const targets = [];
  for (const globalHome of discoverCodexGlobalHomes({ cwd, home, codexHome: codexHome(home), globalHomes })) {
    const configPath = join17(globalHome, "config.toml"), manifestPath = threadLimitManifestPath(globalHome);
    try {
      const configExists = await safeExists(configPath), manifestExists = await safeExists(manifestPath);
      const prior = manifestExists ? await readJson2(manifestPath) : null;
      if (prior && (prior.format !== 1 || prior.owner !== "muster" || prior.configPath !== configPath || typeof prior.before !== "object" || typeof prior.installed !== "object")) {
        throw new Error(`thread-limit ownership manifest is invalid: ${manifestPath}`);
      }
      const ensured = ensureCodexThreadLimits(configExists ? await readSafe(configPath) : "");
      const record = {
        format: 1,
        owner: "muster",
        configPath,
        configCreated: prior?.configCreated ?? !configExists,
        sectionCreated: prior?.sectionCreated ?? ensured.sectionCreated,
        before: prior?.before ?? ensured.before,
        installed: ensured.installed
      };
      targets.push({ globalHome, configPath, manifestPath, text: ensured.text, record });
    } catch (error) {
      throw new Error(`Muster could not update required Codex thread limits at ${configPath}: ${error.message}. Set [agents] max_threads >= 12 and max_depth >= 2, then rerun muster install codex.`);
    }
  }
  return targets;
}
async function prepareThreadLimitRestores({ cwd, home, globalHomes }) {
  const targets = [];
  for (const globalHome of discoverCodexGlobalHomes({ cwd, home, codexHome: codexHome(home), globalHomes })) {
    const manifestPath = threadLimitManifestPath(globalHome);
    if (!await safeExists(manifestPath)) continue;
    const record = await readJson2(manifestPath), configPath = join17(globalHome, "config.toml");
    if (record?.format !== 1 || record.owner !== "muster" || record.configPath !== configPath || typeof record.before !== "object" || typeof record.installed !== "object") {
      throw new Error(`Codex thread-limit ownership manifest is invalid: ${manifestPath}`);
    }
    if (!await safeExists(configPath)) throw new Error(`Muster-owned Codex thread-limit config is missing: ${configPath}`);
    const text = restoreCodexThreadLimits(await readSafe(configPath), record);
    targets.push({ configPath, manifestPath, text, removeConfig: record.configCreated && !text.trim() });
  }
  return targets;
}
...
function validateManagedFiles(manifest, dir, manifestPath) {
  ...
  const base = resolve4(dir), seen = /* @__PURE__ */ new Set();
  for (const file of manifest.files) {
    const destination = typeof file === "string" ? resolve4(base, file) : "";
    if (typeof file !== "string" || file !== basename3(file) || dirname6(destination) !== base || !PROFILE_FILENAME.test(file) || seen.has(file)) {
...
function validateHookManifest(manifest, dir, manifestPath) {
  ...
  for (const file of manifest.files) {
    const destination = typeof file === "string" ? resolve4(base, file) : "";
    const rel = destination ? relative3(base, destination) : "";
    if (typeof file !== "string" || !file || isAbsolute3(file) || rel === ".." || rel.startsWith(`..${sep2}`) || seen.has(file)) {
...
async function prepareHooks({ scope, cwd, home, hookSourceRoot, generation, bootstrapDigest }) {
  const dir = configDir(scope, cwd, home);
  const runtimeDir = join17(dir, "muster"), manifestPath = join17(runtimeDir, MANIFEST), configPath = join17(dir, "hooks.json");
  ...
  const templatePath = join17(hookSourceRoot, "hooks.json");
  ...
  const runtimeScript = join17(runtimeDir, "hooks", "muster-hook.mjs");
  ...
  const sourceFiles = /* @__PURE__ */ new Map([
    ["hooks/muster-hook.mjs", join17(hookSourceRoot, "muster-hook.mjs")],
    ["hooks/action-guard.mjs", join17(hookSourceRoot, "action-guard.mjs")]
  ]);
  const hookHash = createHash3("sha256");
...
function normalizedLocalRoot(value) {
  ...
  return (drive ? `/mnt/${drive[1].toLowerCase()}/${drive[2]}` : resolve4(input)).replace(/\/+$/, "");
}
  ...
  try {
    const canonical2 = async (path) => {
      try {
        ...
        return realpath2(path.toLowerCase());
      }
    };
    const [actualPath, expectedPath] = await Promise.all([canonical2(actual), canonical2(expected)]);
    const [actualStat, expectedStat] = await Promise.all([lstat6(actualPath), lstat6(expectedPath)]);
...
async function trustedMusterMarketplace(item, repoRoot) {
  ...
  return source?.sourceType === "local" && await sameLocalRoot(item.root, repoRoot) && await sameLocalRoot(source.source, repoRoot);
}
...
async function registerPlugin(execFile6, dryRun, repoRoot) {
  ...
  try {
    ...
    if (!marketplace) {
      await run(execFile6, ["plugin", "marketplace", "add", repoRoot]);
...
async function runCodexInstall({ scope = "project", dryRun = false, cwd = process.cwd(), home = homedir7(), repoRoot, execFile: execFile6 = execFileDefault2, scopeLockOptions, globalHomes } = {}) {
  ...
  const pluginRoot = await exists(join17(root, ".codex-plugin", "plugin.json"));
  const selected = pluginRoot ? { metadata: JSON.parse(await readSafe(join17(dirname6(root), "release.json"))), releaseRoot: dirname6(root) } : await resolveCodexRelease(root);
  const distributionRoot = pluginRoot ? resolve4(selected.releaseRoot, "../../../..") : root;
  const pointer = JSON.parse(await readSafe(join17(distributionRoot, ".agents", "plugins", "marketplace.json")));
  ...
  const source = pluginRoot ? join17(root, "agents") : selected.profilesRoot;
  ...
  const dir = agentsDir(scope, cwd, home), manifestPath = join17(dir, MANIFEST);
  ...
  const hookSourceRoot = pluginRoot ? join17(root, "runtime", "install-hooks") : join17(root, "codex", "hooks");
  const hooks = await prepareHooks({ scope, cwd, home, hookSourceRoot, generation: selected.metadata.generation, bootstrapDigest });
  const threadLimits = await prepareThreadLimitInstalls({ cwd, home, globalHomes });
  const managed = new Set(managedFiles.map((file) => resolve4(dir, file)));
  const staleFiles = managedFiles.filter((file) => !files.includes(file));
  for (const file of files) {
    const destination = join17(dir, file);
    if (await safeExists(destination) && !managed.has(resolve4(destination))) throw new Error(`Codex profile conflict: ${destination}. Move it or remove it, then rerun muster install codex.`);
  }
  ...
  const planned = [
    ...files.map((file) => ({ op: "write", path: join17(dir, file) })),
    ...staleFiles.map((file) => ({ op: "remove", path: join17(dir, file) })),
    ...HOOK_FILES.map((file) => ({ op: "write", path: join17(hooks.runtimeDir, file) })),
    ...hooks.staleFiles.map((file) => ({ op: "remove", path: join17(hooks.runtimeDir, file) })),
    { op: "merge", path: hooks.configPath },
    ...threadLimits.flatMap((target) => [{ op: "merge", path: target.configPath }, { op: "write", path: target.manifestPath }])
  ];
  ...
  if (!dryRun) {
    ...
    await withScopeRegistryTransaction(home, async (registry) => {
      ...
      try {
        ...
        for (const target of threadLimits) {
          await snapshot(originals, changed, target.configPath);
          await atomicWriteSafe(target.configPath, target.text);
          await snapshot(originals, changed, target.manifestPath);
          await atomicWriteSafe(target.manifestPath, JSON.stringify(target.record, null, 2) + "\n");
        }
        for (const file of files) {
          const destination = join17(dir, file);
          await snapshot(originals, changed, destination);
          await atomicWriteSafe(destination, await readFile10(join17(source, file), "utf8"));
        }
        ...
        for (const [file, sourcePath] of hooks.sourceFiles) {
          const destination = join17(hooks.runtimeDir, file);
          await snapshot(originals, changed, destination);
...
async function remainingManagedScopes(registry, currentScope) {
  ...
  for (const entry of registry.entries) {
    ...
    const entryAgents = join17(entry.configDir, "agents"), entryManifest = join17(entryAgents, MANIFEST);
...
async function runCodexUninstall({ scope = "project", dryRun = false, cwd = process.cwd(), home = homedir7(), execFile: execFile6 = execFileDefault2, globalHomes } = {}) {
  ...
  const manifestExists = await safeExists(manifestPath);
  const managedFiles = manifestExists ? validateManagedFiles(manifest, dir, manifestPath) : [];
  const files = managedFiles.map((file) => join17(dir, file));
  const hookDir = configDir(scope, cwd, home), hookRuntimeDir = join17(hookDir, "muster"), hookManifestPath = join17(hookRuntimeDir, MANIFEST), hookConfigPath = join17(hookDir, "hooks.json");
  ...
  const hookFiles = hookManifest ? hookManifest.files.map((file) => join17(hookRuntimeDir, file)) : [];
  const present = await codexAvailable({ execFile: execFile6 });
  const ownsScope = manifestExists || hookManifestExists;
  const currentScope = await scopeEntry(scope, cwd, home);
  let liveScopes = [], ownershipCertain = false, removePlugin = false, threadLimits = [];
  let planned = [
  ...
  const uninstallScope = async (registry) => {
    ...
    threadLimits = liveScopes.length === 0 ? await prepareThreadLimitRestores({ cwd, home, globalHomes }) : [];
    planned = [
      ...planned,
      ...threadLimits.flatMap((target) => [{ op: target.removeConfig ? "remove" : "merge", path: target.configPath }, { op: "remove", path: target.manifestPath }])
    ];
    if (dryRun) return;
    ...
    try {
      ...
      for (const target of threadLimits) {
        await snapshot(originals, changed, target.configPath);
        if (target.removeConfig) await removeSafe(target.configPath);
        else await atomicWriteSafe(target.configPath, target.text);
        await snapshot(originals, changed, target.manifestPath);
        await removeSafe(target.manifestPath);
      }
      for (const file of files) {
        await snapshot(originals, changed, file);
        await removeSafe(file);
      }
      if (manifestExists) {
        await snapshot(originals, changed, manifestPath);
        await removeSafe(manifestPath);
      }
      ...
      if (hookManifestExists) {
        ...
        await removeSafe(hookManifestPath);
      }
      if (hookManifest) {
      ...
      if (removePlugin) await run(execFile6, ["plugin", "remove", CODEX_PLUGIN]);
    } catch (error) {
      await restoreFilesystem(originals, changed);
      throw error;
    }
    for (const empty of [join17(hookRuntimeDir, "hooks"), hookRuntimeDir]) try {
...
import { dirname as dirname7, isAbsolute as isAbsolute4, join as join18, parse as parse6, relative as relative4, resolve as resolve5, sep as sep3 } from "node:path";
...
var MCP_TIMEOUT_MS = 5e3;
var mcpVisibilityNote = "Codex may defer MCP tool visibility until lookup or a new session";
function missingPath(error) {
  return error?.code === "ENOENT" || error?.code === "ENOTDIR";
}
async function ordinaryDirectoryPath2(path) {
  const absolute = resolve5(path), root = parse6(absolute).root;
  let current = root;
  for (const part of relative4(root, absolute).split(sep3).filter(Boolean)) {
    current = join18(current, part);
    let stat2;
    try {
      stat2 = await lstat7(current);
    } catch (error) {
...
async function readRegularFile(path, encoding) {
  if (!await ordinaryDirectoryPath2(dirname7(path))) return null;
  let before;
  try {
    before = await lstat7(path);
  } catch (error) {
...
async function readRegularJson2(path) {
  const content = await readRegularFile(path, "utf8");
  return content === null ? null : JSON.parse(content);
}
async function registeredManagedScopes(home) {
  const registryPath = join18(home, "muster", "install-scopes.json");
  let registry;
  try {
    registry = await readRegularJson2(registryPath);
  } catch (error) {
    return { dirs: [], issues: [`could not safely read managed-scope registry ${registryPath}: ${error.message}`] };
  }
  if (!registry) return { dirs: [], issues: [] };
  if (registry.format !== 1 || registry.owner !== "muster" || !Array.isArray(registry.entries)) {
    return { dirs: [], issues: [`managed-scope registry is invalid: ${registryPath}`] };
  }
  const dirs = [], issues = [], seen = /* @__PURE__ */ new Set(), expectedUserScope = resolve5(home);
  for (const entry of registry.entries) {
    const dir = entry?.configDir;
    const valid = entry && ["project", "user"].includes(entry.scope) && typeof dir === "string" && isAbsolute4(dir) && resolve5(dir) === dir && dir.endsWith(`${sep3}.codex`) && (entry.scope !== "user" || dir === expectedUserScope);
    ...
    try {
      const managedDirectories = [dir, join18(dir, "agents"), join18(dir, "muster"), join18(dir, "muster", "hooks")];
      if ((await Promise.all(managedDirectories.map(ordinaryDirectoryPath2))).every(Boolean)) dirs.push(dir);
...
async function runMcpHandshake({ entrypoint, cwd, timeoutMs = MCP_TIMEOUT_MS, spawnProcess = spawn } = {}) {
  return new Promise((resolve7, reject) => {
    ...
    const fail2 = (message) => finish(message instanceof Error ? message : new Error(message));
    try {
    ...
    if (!child?.stdin || !child?.stdout || !child?.stderr) {
      fail2("MCP process did not expose stdio");
      return;
    }
    timer = setTimeout(() => fail2(`MCP initialize/tools/list timed out after ${timeoutMs}ms`), timeoutMs);
    ...
    child.stdout.on("data", (chunk) => {
      ...
      for (; ; ) {
        ...
        try {
          ...
          fail2("MCP emitted invalid JSON-RPC output");
          return;
        }
        if (message.id === 1) {
          if (message.error || !message.result) {
            fail2(`MCP initialize failed: ${message.error?.message || "missing result"}`);
            return;
          }
          initialized = true;
          try {
...
`);
          ...
          if (message.error) {
            fail2(`MCP tools/list failed: ${message.error.message || "unknown error"}`);
            return;
          }
          if (!Array.isArray(message.result?.tools)) {
            fail2("MCP tools/list returned no tools array");
            return;
          }
    ...
    child.stdout.on("error", (error) => fail2(error));
    child.stderr.on("data", (chunk) => {
      stderr += chunk;
    });
    child.stderr.on("error", (error) => fail2(error));
    child.on("error", (error) => fail2(error));
    child.on("exit", (code, signal) => {
      if (!settled) fail2(`MCP process exited before tools/list (${signal || code || "unknown"})${stderr.trim() ? `: ${stderr.trim()}` : ""}`);
    });
    child.stdin.on("error", (error) => fail2(error));
    try {
...
async function runCodexDoctor({ root, cwd = process.cwd(), codexHome: codexHome2, execFile: execFile6, mcpRunner = runMcpHandshake } = {}) {
  const base = root instanceof URL ? fileURLToPath5(root) : root || process.cwd();
  const isPluginRoot = await exists(join18(base, ".codex-plugin", "plugin.json"));
  let selected = null;
  let distributionRoot = base;
  if (isPluginRoot) {
    try {
      const releaseRoot = dirname7(base), metadata = JSON.parse(await readFile11(join18(releaseRoot, "release.json"), "utf8"));
      selected = { generation: metadata.generation, metadata, releaseRoot, pluginRoot: base, profilesRoot: join18(base, "agents") };
      distributionRoot = resolve5(releaseRoot, "../../../..");
    } catch {
    }
  } else {
    try {
  ...
  const plugin = isPluginRoot ? base : selected?.pluginRoot || join18(base, ".agents", "plugins", "releases", "missing", "plugin");
  const checks = [];
  ...
  try {
    const [manifest, pkg] = await Promise.all([
      readFile11(join18(plugin, ".codex-plugin", "plugin.json"), "utf8").then(JSON.parse),
      readFile11(join18(plugin, "package.json"), "utf8").then(JSON.parse)
    ]);
  ...
  try {
    const profileDir = isPluginRoot ? join18(plugin, "agents") : selected.profilesRoot;
  ...
  const missing = [];
  for (const item of required) if (!await exists(join18(plugin, item))) missing.push(item);
  ...
  const userCodexHome = codexHome2 || process.env.CODEX_HOME || join18(homedir8(), ".codex");
  const registeredScopes = await registeredManagedScopes(userCodexHome);
  checks.push({ name: "codex-managed-scopes", ok: registeredScopes.issues.length === 0, detail: registeredScopes.issues.length ? `${registeredScopes.issues.join("; ")}; rerun muster install codex for the affected scope` : registeredScopes.dirs.length ? `${registeredScopes.dirs.length} safe registered managed scope(s) inspected` : "no managed-scope registry found; inspecting current project and user scopes" });
  const scopeHomes = /* @__PURE__ */ new Map();
  for (const [candidate, registered] of [[join18(cwd, ".codex"), false], [userCodexHome, false], ...registeredScopes.dirs.map((dir) => [dir, true])]) {
    let dir;
    try {
      dir = await realpath3(candidate);
    } catch {
      dir = resolve5(candidate);
    }
  ...
  try {
    const handshake = await mcpRunner({ entrypoint: join18(plugin, "runtime", "muster-mcp.mjs"), cwd, timeoutMs: MCP_TIMEOUT_MS });
    ...
    checks.push({ name: "codex-mcp-handshake", ok, detail: ok ? `initialize + tools/list returned ${count}/${CODEX_COUNTS.mcpTools} tools; ${mcpVisibilityNote}` : `initialize + tools/list returned ${count}/${CODEX_COUNTS.mcpTools} tools${handshake?.initialized ? "" : "; initialize did not complete"}; ${mcpVisibilityNote}` });
  } catch (error) {
  ...
  if (selected) {
    ...
    try {
      bootstrapDigest = JSON.parse(await readFile11(join18(distributionRoot, ".agents", "plugins", "marketplace.json"), "utf8")).musterBootstrap?.digest;
    } catch {
    }
    const installations = [];
    for (const dir of hookHomes) {
      try {
        const manifestPath = join18(dir, "agents", ".muster-managed.json");
        const owner = isRegistered(dir) ? await readRegularJson2(manifestPath) : JSON.parse(await readFile11(manifestPath, "utf8"));
  ...
  try {
    selectedBootstrapDigest = JSON.parse(await readFile11(join18(distributionRoot, ".agents", "plugins", "marketplace.json"), "utf8")).musterBootstrap?.digest;
  } catch {
  }
  for (const dir of hookHomes) {
    const manifestPath = join18(dir, "muster", ".muster-managed.json");
    const registered = isRegistered(dir);
    if (!registered && !await exists(manifestPath)) continue;
    try {
      const owner = registered ? await readRegularJson2(manifestPath) : JSON.parse(await readFile11(manifestPath, "utf8"));
      if (!owner) throw new Error(`managed hook manifest is missing: ${manifestPath}`);
      const configPath = join18(dir, "hooks.json");
      const config = registered ? await readRegularJson2(configPath) : JSON.parse(await readFile11(configPath, "utf8"));
      ...
      const runtime = await Promise.all(hookFiles.map((file) => registered ? readRegularFile(join18(dir, "muster", "hooks", file)) : readFile11(join18(dir, "muster", "hooks", file))));
  ...
  const recovery = staleHookScopes.map((dir) => dir === userCodexHome ? `CODEX_HOME=${JSON.stringify(dir)} muster install codex --scope user` : `(cd ${JSON.stringify(dirname7(dir))} && muster install codex --scope project)`).join("; ");
  checks.push({ name: "codex-hooks", ok: Boolean(hookStatus), detail: hookStatus ? `managed lifecycle hooks configured at ${hookStatus}; non-managed hooks require one-time trust review in /hooks` : staleHookScopes.length ? `managed lifecycle hooks are stale or differ from their exact ownership manifest at ${staleHookScopes.join(", ")}. Recovery: ${recovery}` : "managed Codex lifecycle hooks are not installed; run muster install codex for the intended project or user scope" });
...
function validatePython(s) {
  ...
  if (!t) return 0;
  const balanced = (open7, close) => t.split(open7).length - 1 === t.split(close).length - 1;
...
import { join as join19, relative as relative5, extname } from "node:path";
...
async function collectScanFiles(root) {
  ...
  async function walk(dir) {
    ...
    for (const e of ents) {
      ...
      if (e.isDirectory()) {
        const rel = relative5(root, full).replaceAll("\\", "/");
      ...
      files.push({ path: relative5(root, full), content });
    }
  }
  await walk(root);
  return files;
...
import { isAbsolute as isAbsolute6, join as join20 } from "node:path";
...
function parseBacklogRef(text) {
  ...
  if (!/\s/.test(t) && FILE_TOKEN_RE.test(t)) {
    if (isAbsolute5(t) || WINDOWS_DRIVE_RE.test(t) || WINDOWS_UNC_RE.test(t)) {
...
function isTraversalUnsafe(rawSegment) {
  return typeof rawSegment !== "string" || isAbsolute6(rawSegment) || WINDOWS_DRIVE_RE2.test(rawSegment) || WINDOWS_UNC_RE2.test(rawSegment) || rawSegment.includes("..");
}
async function readBacklogCandidate(safeCwd, rawSegment) {
  if (isTraversalUnsafe(rawSegment)) return { readable: false, count: 0 };
  try {
    const content = await readFile13(join20(safeCwd, rawSegment), "utf8");
...
async function detectScope({ cwd = process.cwd(), text = "" } = {}) {
  ...
  if (trimmed === "") {
    const { readable, count } = await readBacklogCandidate(safeCwd, join20(".muster", "backlog.md"));
    if (readable && count > 0) {
...
function readStdin() {
  return new Promise((resolve7, reject) => {
    ...
    process.stdin.on("end", () => resolve7(d));
...
async function main() {
  ...
  try {
    ...
    if (cmd === "detect") {
      ...
      const home = rest.find((a) => !a.startsWith("-") && !consumedValues.has(a)) || homedir9();
      let installed;
...
`));
      ...
      const threshold = thresholdArg === void 0 ? void 0 : Number(thresholdArg);
      if (threshold !== void 0 && (!Number.isFinite(threshold) || threshold < 0 || threshold > 100)) {
        fail("humanize-score --threshold must be a finite number between 0 and 100");
      }
      out(scoreHumanness(text, threshold === void 0 ? {} : { threshold }));
      ...
      const dir = resolve6(rest[0] || process.cwd());
      ...
      const signalsDir = join21(dir, ".muster");
      await mkdir8(signalsDir, { recursive: true });
      await writeFile5(join21(signalsDir, "signals.json"), JSON.stringify(sig, null, 2));
      out(sig);
    } else {
