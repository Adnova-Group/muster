...
B2. **Shared context, once** — run `node ${PLUGIN_ROOT}/runtime/muster.mjs detect .` and `node ${PLUGIN_ROOT}/runtime/muster.mjs capabilities --codex --roles-only` (capture both JSON blobs), plus `node ${PLUGIN_ROOT}/runtime/muster.mjs memory read .muster/memory "<key terms from the backlog>"` and skim any hits — a single time; every item shares the same ProjectProfile and AvailableCapabilities.
B3. **Route every item up front** — per item, in backlog order: run `node ${PLUGIN_ROOT}/runtime/muster.mjs assess --codex "<item text>"`
   ...
   `node ${PLUGIN_ROOT}/runtime/muster.mjs manifest validate --codex .muster/batch/<item-id>.manifest.json` — repair and re-validate
   until `ok: true`.
...
BS2. **Validate** — for every candidate: item format, wave grammar, and dedupe follow `${PLUGIN_ROOT}/internal-skills/interview/SKILL.md`'s
   Decomposition check; the assess-cap and UNMEASURABLE handling below follow `${PLUGIN_ROOT}/commands/capture.md`'s Validate step:
   - **assess-passable** — `node ${PLUGIN_ROOT}/runtime/muster.mjs assess --codex "<item text>"` (every `{key: value}` annotation
...
BS3. **Dedupe** — read `.muster/backlog.md` if it exists; skip any candidate whose text (every `{key: value}`
   ...
   (`${PLUGIN_ROOT}/commands/capture.md` step 3's rule, applied identically here). Track skips for the report.
...
BS5. **Write** — create `.muster/backlog.md` if absent, else append below the existing content, always preserving
   ...
   `$muster-go-backlog`) parse. Record the written items and the skips in STATE.
