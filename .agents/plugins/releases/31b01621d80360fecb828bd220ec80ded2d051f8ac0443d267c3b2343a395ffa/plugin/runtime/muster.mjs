...
import { lstat as lstat3, mkdir as mkdir6, open as open2, readFile as readFile10, rename as rename2, rmdir, unlink } from "node:fs/promises";
...
async function resolveCodexReleaseWithOptions(repoRoot, { retries = 4, readSelections } = {}) {
  ...
  for (const name of candidates) {
    try {
      const record = JSON.parse(await readFile9(join14(repoRoot, ".agents", "plugins", "selections", name), "utf8"));
...
function normalizedLocalRoot(value) {
  ...
  return (drive ? `/mnt/${drive[1].toLowerCase()}/${drive[2]}` : resolve2(input)).replace(/\/+$/, "").toLowerCase();
}
...
async function runCodexUninstall({ scope = "project", dryRun = false, cwd = process.cwd(), home = homedir7(), execFile: execFile6 = execFileDefault2 } = {}) {
  ...
  const otherDir = agentsDir(otherScope, cwd, home), otherManifestPath = join15(otherDir, MANIFEST);
  let otherManaged = false;
  if (await ordinaryDirectoryPath(otherDir)) {
    const otherManifest = await readJson2(otherManifestPath);
...
async function runCodexDoctor({ root, cwd = process.cwd(), codexHome: codexHome2, execFile: execFile6 } = {}) {
  ...
  if (selected) {
    ...
    try {
      bootstrapDigest = JSON.parse(await readFile11(join16(base, ".agents", "plugins", "marketplace.json"), "utf8")).musterBootstrap?.digest;
    } catch {
    }
    const installations = [];
  ...
  checks.push({ name: "codex-hooks-overlap", ok: true, detail: hookStatuses.length > 1 ? "Muster hooks are installed at both project and user scopes; atomic runtime dedupe suppresses identical logical event emissions across copies" : "No project and user Muster hook overlap detected; runtime dedupe remains active for repeated logical events" });
