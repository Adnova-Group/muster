...
Produce a list: an enriched outcome string and testable success criteria, presented for approval via interactive user input — or, when the outcome decomposes into independent parts, a backlog of items (see Decomposition check). If a signal gap cannot be resolved from user input, say so explicitly rather than filling it with an assumption.

Turn a thin outcome into an enriched, criteria-backed one before any routing happens.

## When to run
- Run when `node ${PLUGIN_ROOT}/runtime/muster.mjs assess --codex "<outcome>"` returns `clear: false` (contract:
...
**Callers:** `$muster-plan` invokes this as its front half before routing; `$muster-go` triggers
...
- Ask via the **interactive user input** selection UI: 2-4 labeled options, multiple-choice wherever the answer
...
## Decomposition check
If the outcome spans multiple independent subsystems, surface it via the **interactive user input** selection UI:
...
- **Item format** (must match `$muster-go-backlog`'s parser exactly): exactly one line per item —
...
- **Measurable per item** — each item must embed at least one number or measurable keyword so
  `node ${PLUGIN_ROOT}/runtime/muster.mjs assess --codex "<item text>"` — run with every `{key: value}` annotation stripped
...
- **Then** use **interactive user input** to offer: run the **first item now** (the autopilot lifecycle), run the
  **whole backlog** now (`$muster-go-backlog`), or **just save** (stop here).
...
Render the complete enriched outcome and every success-criteria item inside the approval prompt itself; never refer to unstated criteria as ‘above’ or ‘previous’. Present both for approval via the **interactive user input** selection UI: **Approve** / **Revise** / **Cancel**.
