...
6. If `blocked`: re-dispatch the implementer with the blocker notes, then re-review. Allow **one fix-and-re-review iteration**. If the same blocker remains, ESCALATE to the human with the unresolved blockers.
