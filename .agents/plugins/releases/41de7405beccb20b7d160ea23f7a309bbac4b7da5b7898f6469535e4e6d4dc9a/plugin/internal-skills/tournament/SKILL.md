...
**NATIVE only:** every agent dispatch below uses the Codex subagent dispatcher. No OpenRouter, no external server tools. No human-in-loop step; proceed autonomously through every branch.
...
3. Run the deterministic fusion decision engine:
   ```
   node ${PLUGIN_ROOT}/runtime/muster.mjs fuse .muster/candidates.json .muster/fusion-map.json
   ```
