...
`node ${PLUGIN_ROOT}/runtime/muster.mjs pipeline roadmap`; the pipeline's **prioritize** phase is where step 4 below runs. Run the
...
4. **Score deterministically** — write the initiatives to a JSON file
   ...
   `node ${PLUGIN_ROOT}/runtime/muster.mjs prioritize <file> --model rice`. It returns the items ranked by RICE score
...
5. **Render the roadmap doc** — a now/next/later structure (or ranked tiers) derived from the RICE
   ...
   user-named path.
6. **Optional GitHub issues** — offer (via the **interactive user input** selection UI: how many top items, or
...
7. **Optional GitHub Project board** — offer (same **interactive user input** UI) to also push the prioritized
