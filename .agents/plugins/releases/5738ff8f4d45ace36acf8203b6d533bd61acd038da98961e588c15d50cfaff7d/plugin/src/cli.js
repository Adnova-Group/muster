...
function semverCompare(a, b) {
  const parse5 = (v) => String(v).split(".").map(Number);
  const [aMaj, aMin, aPat = 0] = parse5(a);
  const [bMaj, bMin, bPat = 0] = parse5(b);
...
async function assertPrivateRetirementDirectory(dir) {
  ...
  const unsafeMode = process.platform !== "win32" && ((stat2.mode & 448) !== 448 || (stat2.mode & 63) !== 0);
  if (stat2.isSymbolicLink() || !stat2.isDirectory() || ownerMismatch || unsafeMode) {
...
async function restoreRetiredLock(path, retired, stat2) {
  await assertPrivateRetirementDirectory(dirname4(retired));
...
async function restoreRetiredScopeLock(path, retired, stat2) {
  await assertPrivateScopeRetirementDirectory(dirname5(retired));
