...
3. **Route** — first close any info-gap: `node ${PLUGIN_ROOT}/runtime/muster.mjs assess --codex "$ARGUMENTS"` → `{ clear, signals }`. In attended
   ...
   handling is in the Routine subsection below). Then `node ${PLUGIN_ROOT}/runtime/muster.mjs capabilities --codex` → invoke the **router** skill →
   ...
   **Build the crew FROM `node ${PLUGIN_ROOT}/runtime/muster.mjs capabilities --codex` — never hand-author crew providers.** If `manifest
