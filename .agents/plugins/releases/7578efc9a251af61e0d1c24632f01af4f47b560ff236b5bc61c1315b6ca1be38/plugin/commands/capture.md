...
This reuses the interview skill's Decomposition check machinery **by reference** — same item format, wave grammar, measurability, and dedupe rules `${PLUGIN_ROOT}/skills/interview/SKILL.md` defines for backlog writes. Read it first if unfamiliar, and reuse it exactly rather than re-deriving a divergent format here. Drive:
