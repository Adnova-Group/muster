...
var CLI = path.join(HERE, "..", "src", "cli.js");
...
async function runCli(argv) {
  try {
    const { stdout } = await execFileP("node", [CLI, ...argv], { cwd: process2.cwd(), timeout: 6e4, maxBuffer: 16 * 1024 * 1024 });
...
process2.stdin.on("end", () => process2.exit(0));
