...
async function restoreRetiredLock(path, retirement, stat2) {
  await assertPrivateRetirementDirectory(retirement.dir, retirement);
  const current = await lstat2(retirement.path);
...
async function readScopeRegistry(home) {
  ...
  for (const entry of registry.entries) {
    if (!entry || !["project", "user"].includes(entry.scope) || typeof entry.configDir !== "string" || !isAbsolute2(entry.configDir)) {
...
async function scopeEntry(scope, cwd, home) {
  ...
  try {
    return { scope, configDir: await realpath(dir) };
  } catch (error) {
    if (error.code === "ENOENT") return { scope, configDir: resolve2(dir) };
    throw error;
  }
}
...
function normalizedLocalRoot(value) {
  ...
  return (drive ? `/mnt/${drive[1].toLowerCase()}/${drive[2]}` : resolve2(input)).replace(/\/+$/, "");
}
...
import { lstat as lstat5, open as open4, readFile as readFile12, readdir as readdir10 } from "node:fs/promises";
import { createHash as createHash3 } from "node:crypto";
import { spawn } from "node:child_process";
import { dirname as dirname6, isAbsolute as isAbsolute3, join as join17, parse as parse5, relative as relative3, resolve as resolve3, sep as sep3 } from "node:path";
...
async function ordinaryDirectoryPath2(path) {
  const absolute = resolve3(path), root = parse5(absolute).root;
  let current = root;
  for (const part of relative3(root, absolute).split(sep3).filter(Boolean)) {
...
async function readRegularFile(path, encoding) {
  if (!await ordinaryDirectoryPath2(dirname6(path))) return null;
  let before;
  try {
    before = await lstat5(path);
  } catch (error) {
  ...
  let handle;
  try {
    handle = await open4(path, fsConstants4.O_RDONLY | (fsConstants4.O_NOFOLLOW || 0));
...
async function registeredManagedScopes(home) {
  ...
  const dirs = [], issues = [], seen = /* @__PURE__ */ new Set(), expectedUserScope = resolve3(home);
  for (const entry of registry.entries) {
    const dir = entry?.configDir;
    const valid = entry && ["project", "user"].includes(entry.scope) && typeof dir === "string" && isAbsolute3(dir) && resolve3(dir) === dir && dir.endsWith(`${sep3}.codex`) && (entry.scope !== "user" || dir === expectedUserScope);
...
async function runCodexDoctor({ root, cwd = process.cwd(), codexHome: codexHome2, execFile: execFile6, mcpRunner = runMcpHandshake } = {}) {
  ...
  if (selected) {
    ...
    for (const dir of hookHomes) {
      try {
        ...
        const owner = scopeHomes.get(dir) ? await readRegularJson2(manifestPath) : JSON.parse(await readFile12(manifestPath, "utf8"));
  ...
  for (const dir of hookHomes) {
    ...
    try {
      const owner = registered ? await readRegularJson2(manifestPath) : JSON.parse(await readFile12(manifestPath, "utf8"));
      if (!owner) throw new Error(`managed hook manifest is missing: ${manifestPath}`);
      const configPath = join17(dir, "hooks.json");
      const config = registered ? await readRegularJson2(configPath) : JSON.parse(await readFile12(configPath, "utf8"));
      ...
      const runtime = await Promise.all(hookFiles.map((file) => registered ? readRegularFile(join17(dir, "muster", "hooks", file)) : readFile12(join17(dir, "muster", "hooks", file))));
...
function validatePython(s) {
  ...
  if (!t) return 0;
  const balanced = (open5, close) => t.split(open5).length - 1 === t.split(close).length - 1;
...
import { join as join18, relative as relative4, extname } from "node:path";
...
async function collectScanFiles(root) {
  ...
  async function walk(dir) {
    ...
    for (const e of ents) {
      ...
      if (e.isDirectory()) {
        const rel = relative4(root, full).replaceAll("\\", "/");
      ...
      files.push({ path: relative4(root, full), content });
    }
  }
  await walk(root);
  return files;
...
import { isAbsolute as isAbsolute5, join as join19 } from "node:path";
...
function parseBacklogRef(text) {
  ...
  if (!/\s/.test(t) && FILE_TOKEN_RE.test(t)) {
    if (isAbsolute4(t) || WINDOWS_DRIVE_RE.test(t) || WINDOWS_UNC_RE.test(t)) {
...
function isTraversalUnsafe(rawSegment) {
  return typeof rawSegment !== "string" || isAbsolute5(rawSegment) || WINDOWS_DRIVE_RE2.test(rawSegment) || WINDOWS_UNC_RE2.test(rawSegment) || rawSegment.includes("..");
}
...
function readStdin() {
  return new Promise((resolve4, reject) => {
    ...
    process.stdin.on("end", () => resolve4(d));
