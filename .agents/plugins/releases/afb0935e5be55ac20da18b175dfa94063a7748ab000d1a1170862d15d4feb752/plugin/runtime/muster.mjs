...
async function resolveCodexReleaseWithOptions(repoRoot, { retries = 4, readSelections, lease } = {}) {
  ...
  if (pointer?.name !== "muster" || pluginPath !== STABLE_BOOTSTRAP_PATH || bootstrap?.format !== RELEASE_FORMAT || !GENERATION.test(bootstrap?.initialGeneration || "") || !GENERATION.test(bootstrap?.digest || "")) {
