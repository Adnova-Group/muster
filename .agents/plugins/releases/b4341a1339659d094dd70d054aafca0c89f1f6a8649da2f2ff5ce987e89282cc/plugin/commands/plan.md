...
**Run-active lifecycle:** Write `.muster/run-active` at invocation start (before step 0) — the mode/run-in-progress marker Muster's Codex lifecycle hooks use for state diagnostics. Remove it on Cancel, when handing off to `${PLUGIN_ROOT}/commands/plan-backlog.md` (which owns its own marker from that point forward, exactly like any other delegate hand-off), or when handing off to `$muster-go` (go writes its own on invocation). Codex hooks never delete state markers automatically; on startup, verify and clear only a marker proven stale and owned by the interrupted workflow.

0. **Resolve scope** — run `node ${PLUGIN_ROOT}/runtime/muster.mjs scope "$ARGUMENTS"` → `{ scope, signals }` (empty `$ARGUMENTS` is a valid input here — the scope module's own bare-invocation rules govern; it resolves against a live `.muster/backlog.md` or reports ambiguous).
   ...
   - `scope: "backlog"`, or ambiguous (including the item-but-really-several-deliverables case above) → confirm via the **AskUserQuestion** selection UI, stating the detected `scope` and every string in `signals` **verbatim** (not paraphrased, so the user sees exactly what fired) — options **Plan as ONE outcome** / **Plan as a BACKLOG (batch plan)** / **Cancel**. NEVER silently choose when the signals conflict — this confirm is the only place that decision gets made.
     ...
     - **Plan as a BACKLOG (batch plan)** → delegate — Read `${PLUGIN_ROOT}/commands/plan-backlog.md` and execute its instructions with this backlog/intent, passing `$ARGUMENTS` through unchanged (an empty or raw-intent `$ARGUMENTS` is exactly the bootstrap case plan-backlog.md's own resolve step handles).
...
**Single-outcome path** (steps 1-8 below; unchanged from `$muster-plan`'s front half except the approval hand-off target):

1. **Issue ref?** If `$ARGUMENTS` is a GitHub issue reference (`#N`, a bare number, or an issues URL), run
   `node ${PLUGIN_ROOT}/runtime/muster.mjs issue "$ARGUMENTS"` and re-anchor the returned `outcome` (issue title + body —
...
2. Run `node ${PLUGIN_ROOT}/runtime/muster.mjs assess --codex "$ARGUMENTS"` → `{ clear, signals }`. If `clear: false`, invoke the **interview**
...
3. Run `node ${PLUGIN_ROOT}/runtime/muster.mjs detect .` (pass the explicit path so a drifted cwd doesn't misdetect) and
   `node ${PLUGIN_ROOT}/runtime/muster.mjs capabilities --codex --roles-only`. Capture both JSON blobs.
4. Run `node ${PLUGIN_ROOT}/runtime/muster.mjs memory read .muster/memory "<key terms from the outcome>"` and skim any prior entries.
...
6. The router emits a Crew Manifest. Write it to `.muster/manifest.json`, then validate:
   `node ${PLUGIN_ROOT}/runtime/muster.mjs manifest validate --codex .muster/manifest.json` — repair and re-validate until `ok: true`.
7. Show the manifest to the user (the Glass Box) and collect approval via the **AskUserQuestion** selection UI
   ...
   - **Cancel**: stop immediately; remove `.muster/run-active`.
8. Optionally append a memory entry: `node ${PLUGIN_ROOT}/runtime/muster.mjs memory write .muster/memory <entry.json>`.
