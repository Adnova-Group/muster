...
function semverCompare(a, b) {
  const parse6 = (v) => String(v).split(".").map(Number);
  const [aMaj, aMin, aPat = 0] = parse6(a);
  const [bMaj, bMin, bPat = 0] = parse6(b);
...
import { basename as basename3, dirname as dirname6, isAbsolute as isAbsolute3, join as join16, parse as parse4, relative as relative3, resolve as resolve3, sep as sep2 } from "node:path";
...
var pause = (ms) => new Promise((resolve6) => setTimeout(resolve6, ms));
...
async function ordinaryDirectoryPath(path, { create = false } = {}) {
  const absolute = resolve3(path), root = parse4(absolute).root;
  let current = root;
...
async function scopeEntry(scope, cwd, home) {
  ...
  try {
    ...
    if (error.code === "ENOENT") return { scope, configDir: resolve3(dir) };
    throw error;
  }
}
...
var pause3 = (milliseconds) => new Promise((resolve6) => setTimeout(resolve6, milliseconds));
...
async function privateScopeRetirement(path, { modeCapability = defaultScopeRetirementModeCapability } = {}) {
  for (let attempt = 0; attempt < 8; attempt++) {
    const dir = join16(dirname6(path), `.muster-retired-${process.pid}-${randomUUID3()}`);
    try {
    ...
    return { dir, path: join16(dir, "lock"), stat: stat2, expectedStat: stat2, requirePrivateMode };
  }
...
async function atomicWriteSafe(path, content) {
  ...
  await regularFileState(path);
  const temporary = join16(parent, `.${basename3(path)}.muster-${process.pid}-${Date.now()}-${Math.random().toString(16).slice(2)}.tmp`);
  let handle;
  try {
...
function validateManagedFiles(manifest, dir, manifestPath) {
  ...
  const base = resolve3(dir), seen = /* @__PURE__ */ new Set();
  for (const file of manifest.files) {
    const destination = typeof file === "string" ? resolve3(base, file) : "";
...
function normalizedLocalRoot(value) {
  ...
  return (drive ? `/mnt/${drive[1].toLowerCase()}/${drive[2]}` : resolve3(input)).replace(/\/+$/, "");
}
...
async function runCodexInstall({ scope = "project", dryRun = false, cwd = process.cwd(), home = homedir7(), repoRoot, execFile: execFile6 = execFileDefault2, scopeLockOptions } = {}) {
  ...
  const selected = pluginRoot ? { metadata: JSON.parse(await readSafe(join16(dirname6(root), "release.json"))), releaseRoot: dirname6(root) } : await resolveCodexRelease(root);
  const distributionRoot = pluginRoot ? resolve3(selected.releaseRoot, "../../../..") : root;
  ...
  const managed = new Set(managedFiles.map((file) => resolve3(dir, file)));
  const staleFiles = managedFiles.filter((file) => !files.includes(file));
  for (const file of files) {
    const destination = join16(dir, file);
    if (await safeExists(destination) && !managed.has(resolve3(destination))) throw new Error(`Codex profile conflict: ${destination}. Move it or remove it, then rerun muster install codex.`);
  }
    ...
    await withScopeRegistryTransaction(home, async (registry) => {
      ...
      try {
        ...
        for (const file of files) {
          ...
          await atomicWriteSafe(destination, await readFile10(join16(source, file), "utf8"));
        }
...
import { lstat as lstat7, open as open6, readFile as readFile11, readdir as readdir10 } from "node:fs/promises";
import { createHash as createHash4 } from "node:crypto";
import { spawn } from "node:child_process";
import { dirname as dirname7, isAbsolute as isAbsolute4, join as join17, parse as parse5, relative as relative4, resolve as resolve4, sep as sep3 } from "node:path";
...
async function registeredManagedScopes(home) {
  const registryPath = join17(home, "muster", "install-scopes.json");
  let registry;
  try {
  ...
  const dirs = [], issues = [], seen = /* @__PURE__ */ new Set(), expectedUserScope = resolve4(home);
  for (const entry of registry.entries) {
    const dir = entry?.configDir;
    const valid = entry && ["project", "user"].includes(entry.scope) && typeof dir === "string" && isAbsolute4(dir) && resolve4(dir) === dir && dir.endsWith(`${sep3}.codex`) && (entry.scope !== "user" || dir === expectedUserScope);
    ...
    try {
      const managedDirectories = [dir, join17(dir, "agents"), join17(dir, "muster"), join17(dir, "muster", "hooks")];
...
async function runCodexDoctor({ root, cwd = process.cwd(), codexHome: codexHome2, execFile: execFile6, mcpRunner = runMcpHandshake } = {}) {
  ...
  if (isPluginRoot) {
    try {
      const releaseRoot = dirname7(base), metadata = JSON.parse(await readFile11(join17(releaseRoot, "release.json"), "utf8"));
      selected = { generation: metadata.generation, metadata, releaseRoot, pluginRoot: base, profilesRoot: join17(base, "agents") };
      distributionRoot = resolve4(releaseRoot, "../../../..");
    } catch {
    }
  } else {
    try {
  ...
  try {
    const [manifest, pkg] = await Promise.all([
      readFile11(join17(plugin, ".codex-plugin", "plugin.json"), "utf8").then(JSON.parse),
      readFile11(join17(plugin, "package.json"), "utf8").then(JSON.parse)
    ]);
  ...
  const userCodexHome = codexHome2 || process.env.CODEX_HOME || join17(homedir8(), ".codex");
  ...
  const scopeHomes = /* @__PURE__ */ new Map([[join17(cwd, ".codex"), false], [userCodexHome, false]]);
  for (const dir of registeredScopes.dirs) scopeHomes.set(dir, true);
  const hookHomes = [...scopeHomes.keys()];
  try {
    const handshake = await mcpRunner({ entrypoint: join17(plugin, "runtime", "muster-mcp.mjs"), cwd, timeoutMs: MCP_TIMEOUT_MS });
  ...
  if (selected) {
    ...
    try {
      bootstrapDigest = JSON.parse(await readFile11(join17(distributionRoot, ".agents", "plugins", "marketplace.json"), "utf8")).musterBootstrap?.digest;
    } catch {
    }
    const installations = [];
    for (const dir of hookHomes) {
      try {
        const manifestPath = join17(dir, "agents", ".muster-managed.json");
        const owner = scopeHomes.get(dir) ? await readRegularJson2(manifestPath) : JSON.parse(await readFile11(manifestPath, "utf8"));
  ...
  try {
    selectedBootstrapDigest = JSON.parse(await readFile11(join17(distributionRoot, ".agents", "plugins", "marketplace.json"), "utf8")).musterBootstrap?.digest;
  } catch {
  }
  for (const dir of hookHomes) {
    ...
    try {
      ...
      const configPath = join17(dir, "hooks.json");
      ...
      const runtime = await Promise.all(hookFiles.map((file) => registered ? readRegularFile(join17(dir, "muster", "hooks", file)) : readFile11(join17(dir, "muster", "hooks", file))));
...
import { join as join18, relative as relative5, extname } from "node:path";
...
import { isAbsolute as isAbsolute6, join as join19 } from "node:path";
...
async function readBacklogCandidate(safeCwd, rawSegment) {
  ...
  try {
    const content = await readFile13(join19(safeCwd, rawSegment), "utf8");
...
function readStdin() {
  return new Promise((resolve6, reject) => {
    ...
    process.stdin.on("end", () => resolve6(d));
...
`));
      ...
      const dir = resolve5(rest[0] || process.cwd());
      ...
      const signalsDir = join20(dir, ".muster");
      await mkdir8(signalsDir, { recursive: true });
      await writeFile5(join20(signalsDir, "signals.json"), JSON.stringify(sig, null, 2));
      out(sig);
    } else {
