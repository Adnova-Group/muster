...
**NATIVE only:** every advisor dispatch uses the Codex subagent dispatcher. No OpenRouter, no external server tools. No human-in-loop step; the flow is fully autonomous through every branch (including budget-exhausted -- see step 5).
...
2. **Validate the request.** Run:
   ```
   node ${PLUGIN_ROOT}/runtime/muster.mjs advise .muster/advice-request.json
   ```
...
6. **Feed advice back to the worker.** Re-dispatch the worker via the Codex subagent dispatcher with its original brief PLUS an appended `ADVISOR GUIDANCE` block:
   ```
