---
name: muster-plan
description: "Use for Muster orchestration when the user asks to plan one outcome, assemble and validate a crew manifest, then stop for approval. Explicitly invoke with $muster-plan."
---
...
Use this skill when the request needs to plan one outcome, assemble and validate a crew manifest, then stop for approval. Treat the user's remaining prompt as the outcome or backlog reference.
...
2. Read `${PLUGIN_ROOT}/commands/plan.md` for the authoritative workflow and preserve its approval, isolation, escalation, and receipt gates.
