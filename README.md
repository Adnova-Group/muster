...
| Go-backlog | `/muster:go-backlog <backlog ref>` | The batch clearer. Plain backlogs run sequentially. An annotated `{id}`/`{deps}` backlog builds and reviews every ready item in dependency waves, concurrently when the harness can dispatch safely, then performs disposition/integration in emitted order after the wave barrier. It has one attended stop at the end; an escalated item never aborts the batch. |
