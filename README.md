...
- **`PreToolUse`**: wave-guard, post-run scale gate, and permission gate. While a wave is active (`.muster/wave-active` present), denies main-loop `Edit`/`Write`/`NotebookEdit` and Bash file-write patterns so the orchestrator's iron rule (dispatch via Agent tool, never edit inline) is enforced at the harness level. After a run ends (no marker), a per-turn scale gate denies the 3rd distinct inline file write in a single turn (`MUSTER_INLINE_SCALE`, default 3), counting both editor tools and high-confidence Bash writes so the shell path is not an escape hatch. Trivial 1-2 file turns fall through; orchestration-scale work is routed back to a verb. Both gates honor `MUSTER_WAVE_GUARD` (`off`/`warn`/`deny`). The permission gate runs at every allow/warn path for main-loop calls: destructive commands (`rm -rf`, `git push --force`, and similar) always emit `permissionDecision:"ask"` to force a human re-prompt, even when the key is in an allowlist. Allowlisted non-destructive commands are auto-allowed with a glass-box context line naming the scope. The portable core lives in `permission-policy.js` (no Claude Code dependency). Use `muster allow <toolName> [rest...] [--project]` to capture a permission into the run store (`.muster/allow.run.json`) or the project store (`.muster-allow.json`, tracked, not `.claude/settings.json`).
