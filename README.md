...
| `MUSTER_TODO_GATE` | `deny` | Todo-driving gate on `Task` dispatch during a live run: `deny` blocks a subagent-wave dispatch until a native todo list is written, `warn` allows with a note, `off` disables the gate. |
