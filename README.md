...
- **`PreToolUse`**: wave-guard + post-run scale gate. While a wave is active (`.muster/wave-active` present), denies main-loop `Edit`/`Write`/`NotebookEdit` and Bash file-write patterns so the orchestrator's iron rule (dispatch via Agent tool, never edit inline) is enforced at the harness level. After a run ends (no marker), the advisory nudge alone can't stop drift, so a per-turn scale gate denies the 3rd distinct inline file write in a single turn (`MUSTER_INLINE_SCALE`, default 3), counting both editor tools and high-confidence Bash writes so the shell path isn't an escape hatch. Trivial 1-2 file turns fall through; orchestration-scale work is routed back to a verb. Both gates honor `MUSTER_WAVE_GUARD` (`off`/`warn`/`deny`).
