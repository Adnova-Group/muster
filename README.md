...
| --- | --- | --- |
| Run | `/muster:run <outcome>` | Plans, shows the crew manifest and plan (tasks may carry `owns`/`frozen`/`forbiddenActions` fences and the manifest an overall `mergeDisposition`), then stops for your approval. Approve & run chains into autopilot in-session; Adjust and Cancel stay plan-only. |
