...
| --- | --- | --- |
| Run | `/muster:run <outcome \| backlog ref>` | Plans, shows the crew manifest and plan (tasks may carry `owns`/`frozen`/`forbiddenActions` fences and the manifest an overall `mergeDisposition`), then stops for your approval. Approve & run chains into autopilot in-session; Adjust and Cancel stay plan-only. A backlog ref instead of an outcome switches to batch-plan form: every item is routed up front into ONE batch plan (per-item crew summaries, drain ordering, cross-item fence-conflict flags), and Approve & drain chains into the sprint drain -- nothing executes before that approval. |
...
| Sprint | `/muster:sprint <backlog ref>` | Batch verb: runs the full autopilot lifecycle sequentially over every item in a backlog, ticking each off as it completes, re-resolving the backlog after each item (drain mode) so newly added items join the running batch. An escalated item never aborts the sprint; one attended stop at the end for the batch report. |
| Runner | `/muster:runner [source]` | Unattended one-cycle work-picker for a Claude Code Routine or cron: resumes an answered blocked item or claims exactly one available item, drives it through the full autopilot lifecycle force-coerced to a `pr` disposition, leaves a receipt, and stops. The schedule provides the loop, not the verb. |
...
Run and Autopilot accept a GitHub issue reference (a bare number, `#123`, or an issues URL) as the outcome; Run also accepts the same backlog refs as Sprint (a backlog `.md` path, `issues:<label>`, or `linear:<key>`) to plan a whole batch before anything drains. A thin outcome gets refined first: `muster assess` does a deterministic gap-check, and if the outcome is vague, an interview skill asks one question at a time behind an approval gate before any crew is assembled. An outcome that decomposes into independent parts can instead be written to a sprint backlog (`.muster/backlog.md`) for `/muster:sprint` to run as a batch, or for `/muster:run <backlog ref>` to batch-plan first; `/muster:audit backlog [path]` fills the same backlog from audit's findings, sweeping read-only instead of fixing them inline; `/muster:capture [hint]` fills it a third way, mining a conversation's findings and decisions instead of an audit sweep or an interview decomposition. A backlog item annotated with `{id}`/`{deps}` switches `/muster:sprint` into wave mode: `pr`/`keep` items in a wave dispatch as parallel worktree-isolated runners capped by `MUSTER_SPRINT_PARALLEL`, while `merge-local`/`merge-push` items serialize at the wave barrier. Sprint and Runner share a **coordination** skill (claim/receipt/ledger discipline) so a scheduled Runner and an attended Sprint can safely work the same backlog or `issues:<label>` at once.
...
Muster ships four plugin-native hooks that work together to keep the session anchored:

- **`SessionStart`**: prepends muster's working principles, the six verbs, a one-line project detect, and the default-routing directive at the start of every session, so the session opens with full context and a clear posture for routing actionable prompts through muster. Capture is deliberately not among the six -- it is a backlog generator, not a routed outcome-runner (see [The seven modes](#the-seven-modes)).
- **`UserPromptSubmit`**: fires before each user turn and re-injects a short drift-reinforcement nudge every N turns (default 3) and the full principles payload every N*K turns (default 9), so the session does not revert to default inline Claude behavior over time. A directive-shaped prompt (an imperative verb like fix/build/implement, optionally after a polite lead-in; declaratives like "Update:"/"Fix for" and questions are excluded) also fires the routing-policy reminder immediately the first time it lands with no active muster run -- once per session, then never again until `/clear` re-arms it -- independent of the periodic cadence above.
- **`PreToolUse`**: wave-guard + post-run scale gate + action-class fence. While a wave is active (`.muster/wave-active` present), denies main-loop `Edit`/`Write`/`NotebookEdit` and Bash file-write patterns so the orchestrator's iron rule (dispatch via Agent tool, never edit inline) is enforced at the harness level. After a run ends (no marker), the advisory nudge alone can't stop drift, so a per-turn scale gate denies the 3rd distinct inline file write in a single turn (`MUSTER_INLINE_SCALE`, default 3), counting both editor tools and high-confidence Bash writes so the shell path isn't an escape hatch. Trivial 1-2 file turns fall through; orchestration-scale work is routed back to a verb. A cumulative cross-turn counter tracks distinct inline-edited files across turns too (not just within a turn), warning once per session (never denying) once the running total reaches the same threshold -- catching careful 1-2-file-per-turn drift the per-turn gate never sees; it survives `/compact` and resume, and resets at `SessionStart` (fresh session / `/clear`) and while a run is active. Independently of wave state, whenever `.muster/run-active` and `.muster/forbidden-actions` both exist, the same hook classifies the tool call against the manifest's declared `forbiddenActions` (send/sign/submit/publish/purchase/delete-remote) and denies a match, honoring `MUSTER_ACTION_GUARD` (`off`/`warn`/deny-by-default); the wave-guard, scale-gate, and action-fence checks all honor their own guard variables independently.
- **`PreToolUse` on `Task`/`Agent`** (todo-driving gate): during a live run (`.muster/run-active` present), denies a subagent-wave dispatch unless a native todo list was written since the run started, so plan progress stays visible in Claude Code's todo UI. Biased hard toward allow -- any uncertainty (missing marker, unreadable transcript, unreadable timestamps) allows; honors `MUSTER_TODO_GATE` (`off`/`warn`/`deny`).
...
| `MUSTER_INLINE_SCALE` | `3` | Post-run scale gate threshold. The Nth distinct file write in a single turn with no active wave is denied and routed to a verb (1-2 file turns fall through). Also the cumulative cross-turn threshold: the same count of distinct files edited inline across turns with no active run triggers a one-time per-session warning. |
| `MUSTER_TODO_GATE` | `deny` | Todo-driving gate on `Task`/`Agent` dispatch during a live run: `deny` blocks a subagent-wave dispatch until a native todo list is written, `warn` allows with a note, `off` disables the gate. |
| `MUSTER_ACTION_GUARD` | `deny` | Action-class fence on `PreToolUse` while `.muster/forbidden-actions` is present: `deny` blocks a matching send/sign/submit/publish/purchase/delete-remote tool call, `warn` allows with a reminder, `off` disables the fence. |
...
| `MUSTER_SPRINT_PARALLEL` | `3` | Max concurrent item-runner subagents per wave in `/muster:sprint` wave mode; hard ceiling `8` (higher values clamp, `0` is invalid; concurrency is never unbounded). Read by the sprint mode's orchestration protocol, not by library code. |
