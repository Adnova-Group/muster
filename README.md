...
Every Codex install also updates each detected CLI/Desktop global `config.toml` to keep `[agents] max_threads >= 12` and `max_depth >= 2`. Higher user values and unrelated TOML remain unchanged. On WSL projects under `/mnt/<drive>/Users/<name>/...`, one install covers both the WSL `$CODEX_HOME` and the matching Windows Desktop home. Dry-run writes nothing; uninstall restores only values Muster changed.
...
The Codex plugin bundles the deterministic CLI, all pipelines, 21 MCP tools, 27 custom-agent profiles, 12 public mode/router skills, and 62 resolver-loaded internal capability skills (74 total). The bounded public surface prevents all internal workflows from being injected into every session while retaining deterministic access through Muster's resolver. The npm installer adds Codex-native lifecycle hooks through the supported project or user `hooks.json` layer because Codex 0.144 does not execute plugin-bundled hooks. Codex requires a one-time trust review for these non-managed hooks; inspect them with `/hooks`. The hooks inject orchestration context and surface supported diagnostics and policy warnings. Todo and spawn enforcement remain advisory, and write-capable waves must use isolated Git worktrees.

If `muster doctor --codex` reports split state, run the exact recovery command it prints for every stale scope. For the common host split, refresh WSL with `CODEX_HOME="$HOME/.codex" muster install codex --scope user`, then run `muster install codex --scope user` in Windows PowerShell for `C:\\Users\\<name>\\.codex`. For a stale worktree scope, either run `(cd "<worktree>" && muster install codex --scope project)` or intentionally remove that scope with the matching `uninstall codex --scope project`; restart both clients after recovery.
...
- **`SessionStart`**: prepends muster's working principles, the seven verbs, a one-line project detect, and the default-routing directive at the start of every session, so the session opens with full context and a clear posture for routing actionable prompts through muster. Capture is deliberately not among the seven -- it is a backlog generator, not a routed outcome-runner (see [The eight modes](#the-eight-modes)).
