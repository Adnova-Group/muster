...
Orchestration loops until done via a Ralph-style primitive (`src/loop.js`). `loopState({ iteration, maxIterations, done })` accepts positive safe-integer task budgets up to `TASK_MAX_ITERATIONS = 100` and returns a terminal invalid-budget result for anything else. The review-gate fix-loop uses `reviewGateState` with a default no-progress budget of `REVIEW_GATE_MAX_ITERATIONS = 3` and a non-bypassable `REVIEW_GATE_MAX_TOTAL_ITERATIONS = 12`; dispatch retries similarly default to 2 and stop absolutely at 5. A strictly increasing finite evidence score may extend work beyond the configured no-progress budget but never beyond the absolute ceiling. Flat, regressing, absent, or non-finite progress does not extend it, so repeated identical failures still escalate deterministically.
