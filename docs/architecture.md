...
Orchestration loops until done via a Ralph-style primitive (`src/loop.js`). `loopState({ iteration, maxIterations, done })` returns an object `{ continue: bool, reason: "done" | "max-iterations" | "iterate" }`. The review-gate fix-loop uses the dedicated `reviewGateState` helper with a default no-progress budget of `REVIEW_GATE_MAX_ITERATIONS = 3`; callers may supply an explicit `maxIterations`, and a strictly increasing finite evidence score earns another pass beyond the budget. Flat, regressing, or non-finite progress does not extend it, so repeated identical failures still escalate deterministically. Each wave re-runs implement, review, and fix until the gate passes (`reason: "done"`) or the no-progress budget escalates (`reason: "max-iterations"`), so subagents drive toward the success criteria rather than stopping after one pass.
