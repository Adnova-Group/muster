...
| `MUSTER_BUILD_FORCE=1` rebuild escape hatch | `docs/qa/RUNBOOK.md:249-254` | `node --test --test-name-pattern='MUSTER_BUILD_FORCE' test/codex-build-repro.test.js` | `✔ buildCodexPlugin's version-only skip-if-current check can be bypassed with MUSTER_BUILD_FORCE=1` — 1 pass (test renamed to `...input-digest skip-if-current check...` by the `codex-bundle-cache-key` fix, 2026-07-29 — the `--test-name-pattern` filter still matches by substring and the escape hatch itself is unchanged; this row is the 2026-07-16 receipt verbatim, not re-quoted) |
