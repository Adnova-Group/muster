...
- No dependency-ordered task graph, no wave barrier, no review gate, no tournament or
  ...
  gate/wave layer is what the 31-tool MCP server imports [src: readme-11] [INFERRED].
...
Muster's repository verifies Route A's 31-tool local MCP server and the sequential
...
|---|---|---|---|---|
| Local MCP server (`claude_desktop_config.json`) | Register host-local tools with the agent loop [src: cw-arch] | Route A: the whole 31-tool MCP server [src: readme-11] [src: harness-17] | Tool *results* are advisory to the agent; the server enforces its own contracts (required `dir` on audit, overload rejection, cancellation) [src: mcps-106] | Claude Code loads the full plugin (hooks, commands, agents); and this surface is contradicted for current Cowork / absent in remote sessions [src: cw-connectors] |
