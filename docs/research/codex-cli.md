...
`[CODE-VERIFIED]` muster does not maintain a second tier system for Codex; it keeps Claude-conceptual tiers and translates only at the adapter boundary. `CODEX_MODEL_POLICY` in `src/codex.js` maps (2026-07-18 evidence-receipted retier, PR #74): `haiku → gpt-5.6-terra/high` (long-context-safe locator lane), `sonnet → gpt-5.6-luna/xhigh`, `opus → gpt-5.6-sol/high`, `fable → gpt-5.6-sol/high` (deliberately never routine max effort), plus the cost-based lane `luna-xhigh → gpt-5.6-luna/xhigh` [src: codex-js].
...
`[CODE-VERIFIED]` Per-agent assignments live in the frozen `codex/agents.manifest.json` (27 agents): the `luna-xhigh` budget lane holds 8 bounded roles (`muster-surgeon`, `wsh-api-documenter`, `wsh-tutorial-engineer`, `wsh-test-automator`, plus the human-read content quartet `wsh-business-analyst`/`wsh-content-marketer`/`wsh-customer-support`/`wsh-data-scientist`); the reviewer gate roles (`muster-reviewer`, `wsh-code-reviewer`) carry explicit `model: gpt-5.6-sol`/`high` overrides; Opus-tier builders run sol at `medium`; `test/codex-policy.test.js` pins the full 27-role map and the terra/xhigh reservation invariants [src: manifest-json].
