...
`[CODE-VERIFIED]` `scripts/build-codex.mjs` materializes the complete current plugin: 75 skills (13 public + 62 internal) and 28 MCP tools, plus commands, catalog, pipelines, vendor tree, 27 generated `agents/*.toml` profiles, `.codex-plugin/plugin.json`, and the marketplace template. The public inventory is `$muster-plan`, `$muster-go`, `$muster-plan-backlog`, `$muster-go-backlog`, `$muster-diagnose`, `$muster-audit`, `$muster-runner`, `$muster-capture`, `$muster-init` (Init), legacy `run`/`autopilot`/`sprint`, and the root `$muster` router. Internal skills are ported with Codex bindings; the bundled CLI and Codex MCP adapter are `runtime/muster.mjs` and `runtime/muster-mcp.mjs`, built from `mcp/codex-server.mjs` over the neutral `mcp/server.mjs` core. The Work adapter is built separately as `runtime/chatgpt-work-server.mjs`; neither bundle string-rewrites Cowork source. The legacy `cowork/mcp-server.mjs` and `cowork/chatgpt-work-server.mjs` paths are compatibility shims, not Codex build inputs. Enforced counts live in `CODEX_COUNTS` [src: build-codex] [src: codex-js].
...
| Skills (progressive disclosure, `$` invocation, 2% list budget) | Reusable workflows loaded on demand | 13 public skills (including `$muster-init`) + 62 internal skills; explicit `$muster-*` routing; per-role capability lookups (`--role`) to stay inside the context budget [src: build-codex] [src: codex-build-skills] | Skill selection is model judgment — **advisory**; determinism comes from the skills shelling out to the bundled CLI/MCP brain [src: build-codex] |
