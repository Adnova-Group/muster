...
- Cowork — NOTHING is hookable by an MCP integrator; muster's whole ride is the 29-tool MCP server + protocol injected via MCP `instructions`, with sequential `muster_next` as the verified default and phase-3-gated fan-out. [src: cw-augment] [src: cw-port]
...
- Cowork — the 29-tool MCP server through verified Route A local config (Route B's MCPB descriptor is development-only and not self-contained), MCP `instructions` protocol injection, sequential `muster_next` by default, and phase-3-gated parallel/model-override dispatch. [src: cw-port]
...
| Cowork | 29-tool MCP server, MCP `instructions`, sequential `muster_next` [src: cw-port] | none hookable; server enforces its own contracts [src: cw-augment] | phase-3-gated fan-out, crew dispatch, protocol, STATE (hand-written) [src: cw-augment] | medium — session-mode-dependent, contradictory local-MCP docs [src: cw-mcp] |
...
- cw-port: docs/research/claude-cowork.md §6 — muster's ride; 29-tool MCP server; MCP instructions protocol injection; canonical shared source.
