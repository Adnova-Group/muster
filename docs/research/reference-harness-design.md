...
- Cowork — NOTHING is hookable by an MCP integrator; muster's whole ride is the 31-tool MCP server + protocol injected via MCP `instructions`, with sequential `muster_next` as the verified default and phase-3-gated fan-out. [src: cw-augment] [src: cw-port]
...
- GPT Work / Agents SDK — ChatGPT Work compatibility is unverified pending a Work-mode load probe; in a built SDK lane, `needs_approval` + guardrails + durable `RunState` are HARD once declared but nothing gates by default. [src: gw-hitl] [src: gw-verdict]
...
- Cowork — the 31-tool MCP server through verified Route A local config (Route B's MCPB descriptor is development-only and not self-contained), MCP `instructions` protocol injection, sequential `muster_next` by default, and phase-3-gated parallel/model-override dispatch. [src: cw-port]
...
- GPT Work — unverified product lane pending a Work-mode load probe; the Agents SDK would be a build (needs_approval + RunState + Sandbox agents + custom Session backends), not an augmentation. [src: gw-verdict]
...
| Cowork | 31-tool MCP server, MCP `instructions`, sequential `muster_next` [src: cw-port] | none hookable; server enforces its own contracts [src: cw-augment] | phase-3-gated fan-out, crew dispatch, protocol, STATE (hand-written) [src: cw-augment] | medium — session-mode-dependent, contradictory local-MCP docs [src: cw-mcp] |
...
| GPT Work | Product compatibility unverified; Work-mode load probe required; Agents SDK for a built lane [src: gw-verdict] | needs_approval + guardrails (opt-in, SDK only) [src: gw-hitl] | Work extension surfaces and Workspace-agents retrieval [src: gw-verdict] | probe (Work) / build (SDK) [src: gw-verdict] |
...
8. **`codex-install-thread-limits`: historical gap vs current enforcement.** **Resolved, landed, and migrated for 0.146:** `ensureCodexThreadLimits` ensures canonical `max_concurrent_threads_per_session` exists, defaults to `12` only when no user ceiling exists, and never raises or lowers a positive user value. The final managed-scope uninstall calls `restoreCodexThreadLimits` to restore only Muster's recorded change; legacy values are cleaned only when an older valid Muster receipt proves ownership. [src: dr-install] [src: dr-audit] [src: cx-subagents]
...
- cw-port: docs/research/claude-cowork.md §6 — muster's ride; 31-tool MCP server; MCP instructions protocol injection; canonical shared source.
...
- gw-verdict: docs/research/gpt-work.md §1.1, §9 — ChatGPT Work compatibility is unverified pending a Work-mode load probe; per-candidate harness-target verdict.
