...
| muster construct | native replacement | win | risk | effort |
|---|---|---|---|---|
| the whole ride (31-tool MCP server + `instructions` protocol injection, sequential by default) | **possibly** native plugins — Cowork now bundles skills/hooks/subagents in the Claude Code plugin format [src: cw-plugins] | if `plugin/` loads under Cowork's loader, muster rides skills-as-`/`-menu natively instead of MCP-only [src: cw-plugins] | **unverified** — the single highest-value open question; adapter's "no plugin/skill/hook primitives" claim is stale; parallel/model override also requires a fresh phase-3 receipt [src: cw-plugins] | **probe (L)** |
...
| muster construct | native replacement | win | risk | effort |
|---|---|---|---|---|
| orchestrator wave dispatch | `codex-wave` launches one authenticated `codex exec -C` process per registered pristine worktree; versioned `spawn_agent` packets remain available only for explicit non-wave leaf delegation [src: cx-subagents] | production writers get a pinned cwd, trusted runner policy, bounded process tree, and all-process barrier | no native cwd field exists on subagent dispatch, so it cannot serve as the production wave lane [src: cx-subagents] | done |
...
1. **cowork-plugin-loader-probe** `[Cowork | thins: 31-tool MCP-only adapter]` — Hands-on phase-3 probe
