...
**Goal:** Publish and consume coherent immutable Codex generations through one atomic marketplace pointer, while generated workflows enforce subagent watch-to-completion behavior.

**Architecture:** A shared release module validates ordinary-file trees, computes canonical content hashes, publishes immutable releases, atomically switches `marketplace.json`, and resolves the selected plugin/profile pair. The generator builds only in staging and all consumers resolve the pointer. Codex-only generated workflows receive a common wait/list/receipt invariant.
...
- The marketplace manifest is the only consumer-visible switch.
...
- Do not modify hook deduplication until the separate debugger reports.
...
- [ ] Write failing tests for canonical hashing, Windows-shaped/traversal paths, symlinks/special files, exact pointer resolution, atomic swap failure, and coherent profile/plugin selection.
- [ ] Run `node --test test/codex-release.test.js`; expect failures because the module is absent.
- [ ] Implement strict `lstat`/containment validation, canonical metadata, immutable release validation, and one-file pointer publication.
...
- [ ] Generate exclusively in sibling staging inside `try/finally`, validate all copied sources, publish the immutable release, then switch the pointer.
...
- [ ] Write failing checks requiring `collaboration.wait_agent`, `collaboration.list_agents`, receipt recording, next-eligible dispatch, completion refusal, valid stops, and advisory-hook wording in every generated surface.
...
**Files:**
- Modify: `.muster/STATE.md` locally only.
...
- [ ] Append RED/GREEN/mutant/full-suite evidence to STATE.
