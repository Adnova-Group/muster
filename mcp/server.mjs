#!/usr/bin/env node
// App-neutral muster MCP core. Thin host adapters select Cowork, Codex, or ChatGPT Work.
//
...
const MUSTER_MCP_HOST = process.env.MUSTER_MCP_HOST;
if (!["cowork", "codex", "work"].includes(MUSTER_MCP_HOST)) {
  process.stderr.write("muster-mcp: MUSTER_MCP_HOST must be cowork, codex, or work\n");
  process.exit(1);
}
...
const SPRINT_PROTOCOL_PATH = existsSync(path.join(HERE, "..", "cowork", "sprint-protocol.md"))
  ? path.join(HERE, "..", "cowork", "sprint-protocol.md")
  : path.join(HERE, "sprint-protocol.md");
...
const CODEX_PROTOCOL = COWORK_PROTOCOL
  .replace(
    "Running muster here: you have these MCP tools. No skills or slash commands, so follow this protocol directly. The verified default is sequential dispatch through muster_next. Use parallel fan-out or a per-call model override only after a phase-3 probe receipt from this active Cowork build proves those capabilities.",
...
const WORK_PROTOCOL = [
  "Running Muster in ChatGPT Work: use these deterministic MCP tools for routing, scoring, validation, and wave computation.",
  "Treat the selected tool profile as the capability boundary. Do not call or imply access to tools outside the advertised list.",
].join("\n");
const HOST_PROTOCOL = MUSTER_MCP_HOST === "cowork" ? COWORK_PROTOCOL
  : MUSTER_MCP_HOST === "codex" ? CODEX_PROTOCOL
  : WORK_PROTOCOL;
const INSTRUCTIONS = [PRINCIPLES, VERBS, ROUTING_POLICY, HOST_PROTOCOL].join("\n\n");
...
if (MUSTER_MCP_HOST === "codex") {
  TOOLS.muster_capabilities = {
    ...TOOLS.muster_capabilities,
    argv: ["capabilities", "--codex"],
    description: "Resolve every Muster role against enabled Codex plugins, skills, MCP servers, and custom-agent profiles. Each agent-backed role also carries codexModel {model, effort}, resolved from the committed .codex/agents mapping.",
  };
  TOOLS.muster_capabilities_roles = {
    ...TOOLS.muster_capabilities_roles,
    argv: ["capabilities", "--codex", "--roles-only"],
    description: "Resolve every Muster role against Codex and return only the compact {roles} map.",
  };
  TOOLS.muster_assess = { ...TOOLS.muster_assess, argv: ["assess", "--codex"] };
} else if (MUSTER_MCP_HOST === "work") {
  TOOLS.muster_capabilities = {
    ...TOOLS.muster_capabilities,
    argv: ["capabilities"],
    description: "Resolve every Muster role using the capabilities visible to this ChatGPT Work connection.",
  };
  TOOLS.muster_capabilities_roles = {
    ...TOOLS.muster_capabilities_roles,
    argv: ["capabilities", "--roles-only"],
    description: "Resolve every Muster role for this ChatGPT Work connection and return only the compact {roles} map.",
  };
}
...
if (TOOL_PROFILE === "chatgpt-work-pro-safe") {
  ...
  const nonce = process.env.MUSTER_MCP_PROBE_NONCE;
  const attestationPath = process.env.MUSTER_MCP_PROBE_ATTESTATION_PATH;
  let identity;
  try { identity = JSON.parse(process.env.MUSTER_MCP_PROBE_IDENTITY); } catch { identity = null; }
  const serverInstanceId = process.env.MUSTER_MCP_PROBE_SERVER_INSTANCE_ID;
  if (
    !identity
...
async function callProbe(args, signal) {
  ...
  try {
    ...
    if (parent.isSymbolicLink() || `${parent.dev}:${parent.ino}` !== process.env.MUSTER_MCP_PROBE_PARENT_ID) {
  ...
  try {
    ...
    const { stdout } = await new Promise((resolve, reject) => {
      execFile("node", [CLI, ...argv], {
        ...
        env: { ...process.env, MUSTER_RUNTIME: MUSTER_MCP_HOST },
