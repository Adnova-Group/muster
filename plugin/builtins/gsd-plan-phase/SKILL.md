...
muster_builtin: true
adapted_from: open-gsd/gsd-core gsd-core/workflows/plan-phase.md
license: MIT
---
...
**Interaction with `--prd <filepath>`.** `--mvp` and `--prd` compose. The PRD express path (Step 3.5) creates `CONTEXT.md` from the PRD file and continues to research; the Walking Skeleton gate fires independently from the conditions above. When both are active on Phase 1 of a new project, the planner receives `WALKING_SKELETON=true` and PRD-derived context simultaneously — the PRD informs *what the skeleton should prove*. No precedence is needed; the two signals are orthogonal. See the pinned upstream [`mvp-concepts.md`](https://github.com/open-gsd/gsd-core/blob/18995380ce8d85bccd8a3c64973664348b29cab8/gsd-core/references/mvp-concepts.md) reference for the broader interaction map.
