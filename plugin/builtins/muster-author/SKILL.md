---
name: muster-author
description: Built-in authoring provider — draft persuasive, audience-first copy using proven frameworks. Used by content/doc pipelines for the author role.
muster_builtin: true
adapted_from: Muster (copywriting frameworks — AIDA / PAS / BAB / QUEST, E-E-A-T)
license: Apache-2.0
---
...
- **Pick a framework and follow it.** AIDA (Attention→Interest→Desire→Action), PAS
  (Problem→Agitate→Solution), BAB (Before→After→Bridge), or QUEST
  (Qualify→Understand→Educate→Stimulate→Transition). State which you used.
