...
1. **Seed** — `npx muster audit` -> Crew Manifest at `.muster/manifest.json`; validate (`npx muster manifest validate`).
2. **Branch** — create a work branch off the base (never run on the base branch).
3. **Parallel dimension sweep** — dispatch the chosen provider per dimension CONCURRENTLY, each READ-ONLY, on its role's model (architecture-review on opus, etc.): architecture, tech-debt, coverage, simplification, readability, security. Each returns findings: severity (P0/P1/P2), location (file:line), problem, suggested fix.
