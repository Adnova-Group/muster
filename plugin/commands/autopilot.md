...
**Run-active lifecycle:** Write `.muster/run-active` at invocation start (before step 0) -- the mode/run-in-progress marker the `PreToolUse` hook uses to scope the scale-gate. Remove it after step 8 (merge decision) or on escalation exit. `SessionStart` on a fresh session clears a stale marker automatically.
