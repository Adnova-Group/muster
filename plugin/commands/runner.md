...
1. **Resolve the source** — `$ARGUMENTS` empty or a path defaults to `.muster/backlog.md` (Binding B); `issues:<label>` uses Binding A. A missing file or `gh` failure is reported and the cycle stops — nothing claimed.
2. **Resume check** — scan BLOCKED items for an answer BEFORE any new claim: Binding B — a STATE `ANSWER <slug>: ...` line newer than the matching `BLOCKED ... <slug>` receipt; Binding A — a non-`MUSTER `-prefixed (human) reply newer than the `MUSTER BLOCKED` comment. An answered item resumes (claim receipt noted) in place of step 3, then goes straight to step 4.
