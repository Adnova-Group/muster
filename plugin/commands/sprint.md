...
1. **Resolve the backlog** — `$ARGUMENTS` is either:
   ...
   Once resolved, run `npx -y @adnova-group/muster sprint-waves <file>`: any item's `{id: ...}`/`{deps: ...}` annotation switches the sprint to **wave mode** (below) and the verb's JSON is authoritative — `ok:false` reports the named errors and stops, nothing runs. No such annotations → the sequential steps 2-4 below, unchanged.
...
Triggered when step 1's `sprint-waves` call reports `{id}`/`{deps}` annotations; waves replace steps 2-3's flat queue — step 4's single attended stop and both step 3 overrides still apply to every item.
