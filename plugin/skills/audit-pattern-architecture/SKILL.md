...
- (2026-08-04, source: docs/decisions/codex-install-lock-divergence.md — the ≥250-line unification premise was falsified only by reading both protocols side by side) Lock-protocol census: enumerate every distinct on-disk lock/sentinel state machine (`grep -rn '\.recover\|\.muster-transition\|\.acquire-\|muster-reclaim-\|LockText\|lockIsStale\|staleScopeLock' src/*.js`); each protocol must either import `codex-lock.js`/`fs-safe.js` primitives or cite a divergence decision record naming its deliberate semantic deltas (fail-closed vs fail-open corruption, hard-expiry vs never-reclaim-live-owner, public vs private hook seams). A NEW lock implementation with neither is the finding. — false-positive note: `codex-install-scope-lock.js`'s divergence is documented and deliberate (its header cites the decision record); a cited record clears the protocol, don't re-litigate it.
- (2026-08-04, source: scoped-audit-shakedown dead-weight feedback + live probe: 29 raw `rename(` sites outside fs-safe.js vs 10 `atomicWrite` importers) Atomic-write routing sweep: for each temp-write-then-rename outside `src/fs-safe.js`, verify (a) staging file is same-directory (cross-device rename fails), (b) the failure path unlinks the staged temp, (c) whether `atomicWrite`/`withFileMutationLock` should own it outright. — false-positive note: renames that ARE the lock protocols' own intermediate artifacts (`.acquire-*` link-into-place, quarantine moves) are the state machine, not a bypass.
- (2026-08-04, source: `src/codex-install-shared.js` snapshot exports + live probe: 41 `exactFileSnapshot`/`sameExactFileSnapshot` sites) Snapshot-verify-commit pairing: for every mutation path that takes a snapshot before staging, confirm a `sameExactFileSnapshot`/`sameInode` re-verify runs after staging and BEFORE publish (the rename/link into place); a snapshot captured but never re-compared pre-commit is TOCTOU-shaped even inside a lock. — false-positive note: snapshots used purely for rollback/retirement-receipt bookkeeping need no pre-publish compare; check which side of the publish the snapshot serves.
