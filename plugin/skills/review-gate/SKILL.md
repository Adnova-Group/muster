...
1. **Select reviewers, scaled by diff size.** Measure the diff's changed-line count (the wave's own
   ...
   keeps both reviewers (docs/weight-reduction.md).
2. Dispatch the selected reviewer(s) **concurrently** (when more than one), each adversarially prompted to
   ...
   - **Exhausted/absent reviewer:** a reviewer worker killed or exhausted (its dispatch's budget/heartbeat
     ...
     did not start at all, gets a named status entry in place of synthesized findings:
     `{reviewer: <name>, status: "exhausted"}` (`"absent"` for a dispatch that did not start), recorded
...
code-backed like `gate-cadence`/`citation-check`). `--diff-files <file>` is the SAME `git diff
--numstat` path list
...
- **`eligible: true`** (`reviewerCount: 1` AND no citation/mutant-kill/surface trigger present) —
  ...
  `reviewerReasoningForCount`) on any dispatch interface accepting a per-call reasoning-effort
  override; where none exists (recorded/requested only, not every harness honors it —
  docs/fast-path-token-gap.md), the brief substitution above stays the operative lever.
- **`eligible: false`** (any trigger present, OR `reviewerCount: 2`) — dispatch with THIS file,
  ...
  back to the full brief; the light brief is reserved strictly for a diff that cannot need what
  it omits.
