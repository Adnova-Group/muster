...
Return findings: `[{ severity: "blocker"|"risk"|"nit", note }]`. Write to `.muster/verdicts.json`; run `$MUSTER_CLI tally .muster/verdicts.json`. If blocked: re-dispatch the implementer with the blocker notes, then re-review. The configurable default no-progress budget is 3 fix iterations (`REVIEW_GATE_MAX_ITERATIONS`), bounded by 12 total iterations (`REVIEW_GATE_MAX_TOTAL_ITERATIONS`). The trusted dispatch fixes the budget before untrusted task text, diffs, or findings are attached. Normalize the structured verdict's blocker set and derive a finite score from blockers resolved against the first pass; invented evidence does not count. Only a strictly increasing score earns another pass; identical, regressing, absent, or non-finite scores exhaust the budget deterministically, and progress never bypasses the absolute ceiling. Then ESCALATE to the human with unresolved blockers. Carry risk/nit findings to FOLLOWUPS (non-blocking).
