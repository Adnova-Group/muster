...
function brief({ name, correct, input, expected, cwd, baseSha, blocker }) {
  ...
  return [
    ...
    "Inspect the repository and retain this exact context. If no reviewer blocker is included below, create .muster/STATE.md recording that implementation is awaiting review, commit that bookkeeping so the worktree is pristine, and do not edit source or tests. If a reviewer blocker is included, repair it, run node --test test/operation.test.js, and commit the green change so the worktree is pristine.",
    context,
    "</remote-text>",
...
const BENCHMARK_HARNESS = "real Codex paired benchmark through production runCodexWave and authenticated runCodexWaveContinuation";
const evidence = {
  ...
  metric: "primary context-input metric = per-fix uncached input tokens (input_tokens - cached_input_tokens, with seed counters subtracted from cumulative resume counters); raw total input tokens are retained as a secondary reported metric; time-to-fix includes the production invocation and post-fix focused verification",
  productionPath: {
...
try {
  ...
  if (checkpoint?.harness === BENCHMARK_HARNESS && checkpoint?.productionPath
    && Array.isArray(checkpoint.cases) && checkpoint.cases.length <= caseCount
    && checkpoint.cases.every((entry, index) => entry?.case === tasks[index][0])) {
    evidence.generatedAt = checkpoint.generatedAt;
    evidence.cases = checkpoint.cases;
  }
} catch (error) {
...
try {
  const completedCount = evidence.cases.length;
  for (const [offset, task] of tasks.slice(completedCount, caseCount).entries()) {
    const index = completedCount + offset;
    ...
    let seedStatus = run("git", ["status", "--porcelain=v1", "--untracked-files=all", "--ignored=matching"], {
      cwd: fixture.resumed,
    }).trim();
    let seedNormalization = null;
    if (seedStatus === "?? .muster/STATE.md") {
      await rm(join(fixture.resumed, ".muster", "STATE.md"));
      seedNormalization = { removedUntrackedAwaitingReviewState: true };
      seedStatus = run("git", ["status", "--porcelain=v1", "--untracked-files=all", "--ignored=matching"], {
        cwd: fixture.resumed,
      }).trim();
    }
    if (seedStatus) throw new Error(`${name}: seed turn did not leave a pristine worktree:\n${seedStatus}`);
