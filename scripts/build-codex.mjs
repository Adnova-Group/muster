...
// The adapter is the canonical watch-policy source. Generated skills embed the exact section,
// preventing the adapter and per-mode surfaces from drifting independently.
const adapterSource = readFileSync(new URL("../codex/skill-adapter.md", import.meta.url), "utf8");
const agentWatchStart = adapterSource.indexOf("## Agent watch invariant");
if (agentWatchStart < 0) throw new Error("codex adapter agent watch section not found");
const agentWatchProtocol = adapterSource.slice(agentWatchStart).trimEnd();
