...
function adaptCommandForCodex(text, name, contract) {
  ...
  if (name === "audit.md") {
    ...
    const capacitySweep = [
      ...
      "   Dispatch these three briefs concurrently when the configured Codex capacity permits, otherwise in dependency-free batches. Respect `agents.max_concurrent_threads_per_session`; neither lower nor raise it. Every worker uses `fork_turns: \"none\"`, an explicit task budget (25 steps by default, configurable upward for a cohesive task), focused commands only, and one concise receipt. Add prompt-quality as a fourth read-only brief only when the scoped diff changes prompts or agent instructions. Consolidation is forbidden until each required dimension has a receipt."
    ].join("\n");
