...
function adaptCommandForCodex(text, name, contract) {
  ...
  if (name === "audit.md") {
    ...
    const capacitySweep = [
      ...
      "   Dispatch these three briefs concurrently when the configured Codex capacity permits, otherwise in dependency-free batches. Respect `agents.max_concurrent_threads_per_session`; neither lower nor raise it. Every worker uses `fork_turns: \"none\"`, focused commands, a deterministic progress fingerprint, a configured no-progress threshold, and one concise receipt. Add prompt-quality as a fourth read-only brief only when the scoped diff changes prompts or agent instructions. Consolidation is forbidden until each required dimension has a receipt."
    ].join("\n");
