...
function adaptCommandForCodex(text, name, contract) {
  ...
  let result = translatePluginPaths(translateCodexProse(text))
    ...
    .replace(registryFallbackRe, "construct the complete wave and first run it through `node ${PLUGIN_ROOT}/runtime/muster.mjs codex-wave <wave.json>` with trusted repository/base inputs supplied out of band; never spawn directly from the backlog or manifest. For each returned non-process packet only, call the model-version-resolved spawn -- `collaboration.spawn_agent` on v2 models, `multi_agent_v1.spawn_agent` on v1 -- exactly as returned, with `agent_type: \"muster-runner\"`. " + contract.forkTurns + " `agent_type` is a Codex runtime extension and may be absent from the simplified displayed signature; include it anyway. Only an actual rejected tool call proves the profile unavailable. If that call rejects the type, fail the item closed with a profile-registration diagnostic and remediation to reinstall/start a new session; do not silently use a generic agent because that loses the pinned role/model policy")
...
function adaptOrchestratorForCodex(text, contract) {
  ...
  result = result.slice(0, waveDispatchStart)
    + `${waveDispatchHeading}\n\nCodex has no counterpart to Claude Code CLI's agent-teams \`Workflow\` tool. Every production wave MUST first run through \`node ${"${PLUGIN_ROOT}"}/runtime/muster.mjs codex-wave <wave.json>\`; never choose or invoke \`spawn_agent\` directly from a manifest. That runtime is the single fail-closed lane selector: verified disjoint single-regular-file fences produce bounded, version-resolved subagent packet batches, while overlapping, omitted, malformed, directory, missing, case-ambiguous, symlinked, or hard-linked fences run as one hermetic \`codex exec -C\` process per member. The process-wave input must name a distinct registered linked worktree root for every member. The runtime canonicalizes and verifies every path, rejecting absent, nested/wrong, base-checkout, unregistered, or duplicate worktrees before it probes or starts Codex; once validated, it launches the members concurrently and owns the all-process barrier.\n\n**The dispatch and barrier shapes are VERSION-DEPENDENT** for the in-session lane: Codex resolves its subagent API per MODEL from the catalog's \`multi_agent_version\`, so never hardcode one shape -- use only the packets returned by \`codex-wave\` and fail closed to v1 rather than guessing v2.\n\n${contract.shapesTable}\n\n\`wait_agent\` BLOCKS until something happens, so there is no interval to tune and nothing to tight-poll -- call it in a loop until every dispatched member has settled (neither version is an all-barrier), and take receipts from the mailbox, not \`list_agents\`. \`node ${"${PLUGIN_ROOT}"}/runtime/muster.mjs wave-dispatch\` always resolves \`mode: "prose"\` on this harness (there is no Codex-side \`--agent-teams\`/\`MUSTER_AGENT_TEAMS\` declaration path); dispatch only the non-process packets returned by \`codex-wave\` through the version-resolved spawn exactly as described above -- gated by this session's OWN \`multi_agent\` capability, declared not auto-probed same as every other check here: Codex ships \`multi_agent\` default-on, so only an explicit \`multiAgent: false\` (or \`MUSTER_CODEX_MULTI_AGENT=0\`) drops dispatch to \`mode: "sequential-inline"\` -- one crew member at a time, never a partial/mixed fan-out.\n\nNative review routing stays disabled: the paid native-review shadow benchmark rejected adoption with 0/10 schema-valid outputs. Production review continues through muster's canonical independent review gate; never route a wave verdict through \`codex review\`.\n\n### Worktree isolation: registered process roots or receipts-only\n\nThe process-wave lane enforces registered linked worktrees mechanically before execution. Codex subagent dispatch has no cwd field on either API version, so the in-session lane has no native worktree mechanism to select: run \`node ${"${PLUGIN_ROOT}"}/runtime/muster.mjs worktree-isolation --harness codex\` to confirm it resolves \`mechanism: "receipts-only"\`. The brief's absolute \`WORKTREE CWD\` (Provider and model policy, above) plus a base-SHA receipt per dispatched crew member -- \`{taskId, mechanism, baseSha, worktreePath}\`, refused over a missing or non-hex \`baseSha\` -- stand in for the isolation guarantee muster cannot get from in-session dispatch; append the receipt to STATE alongside the dispatch line. Immediately after, run \`node ${"${PLUGIN_ROOT}"}/runtime/muster.mjs receipt-verify <baseSha> --cwd <absolute worktree path>\` and treat a nonzero exit as a receipt failure -- escalate it, never continue silently.\n\n`
    + `Process-wave invocation is incomplete unless it appends \`--repository-root <trusted repo root> --base-sha <exact full base SHA>\` to the command above. Those trusted values travel out of band from \`wave.json\`; the runtime binds every member to that shared common Git directory and exact base, then revalidates immediately before each bounded launch.\n\n`
