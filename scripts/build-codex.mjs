...
function adaptOrchestratorForCodex(text, contract) {
  ...
  const provider = `      - **Provider and model policy:** look up only the needed role with \`node ${"${PLUGIN_ROOT}"}/runtime/muster.mjs capabilities --codex --role <role>\`; do not reprint the full inventory. Resolve a skill provider with \`node ${"${PLUGIN_ROOT}"}/runtime/resolve-skill-provider.mjs <chosen.source> <chosen.id>\` when needed. Put the verified provider instructions into that member's \`prompt\`, its resolved \`codexModel\` into optional \`model\`, and its distinct pristine registered worktree into \`cwd\`. Never call a subagent API from this production-wave step. Include a 25-step ceiling, one-follow-up maximum, focused-test-first rule, absolute manifest/STATE paths, owned and frozen paths, dependency receipts, and the return contract in the prompt. Assemble all members before invoking \`codex-wave\`; trusted repository root/base SHA stay out of the manifest and are passed only as CLI flags. The runtime applies the trusted thread ceiling, launches every member with hermetic \`codex exec -C\`, and owns the barrier.\n`;
