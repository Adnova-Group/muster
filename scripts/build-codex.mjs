...
function adaptCommandForCodex(text, name, contract) {
  ...
  if (name === "audit.md") {
    ...
    const capacitySweep = [
      ...
      "   Dispatch these three briefs concurrently when the configured Codex capacity permits, otherwise in dependency-free batches. Respect `agents.max_concurrent_threads_per_session`; neither lower nor raise it. Before each dispatch, inspect THIS active session's callable namespaces (`multi_agent_v1.spawn_agent` means v1; `collaboration.spawn_agent` means v2) and registered agent ids, then run `node ${PLUGIN_ROOT}/runtime/muster.mjs codex-audit-provider --role <role> --task-id <id> --callable-apis <v1,v2> --callable-agent-ids <id,...> [--api-override <v1|v2>] --message-file <brief>`. Supply `--api-override` only when the active runtime reports one. The CLI derives the role's full manifest-ordered independent-agent chain, intersects live inventory with the active session registry, and resolves each profile's effective override-before-catalog API; dispatch ONLY its returned packet, unchanged. It preserves the preferred provider when compatible, skips stale incompatible/unavailable preferences for the first independent compatible provider, and emits `{mode:\"inline\", packet:null, degradation:{code:\"CODEX_AUDIT_NO_COMPATIBLE_PROVIDER\",...}}` only when none exists. Append that machine-readable degradation to STATE; never construct or send a packet for a namespace absent from this session. Every worker gets the packet's version-correct no-context field, an explicit cohesive-task budget, focused commands, a deterministic progress fingerprint, a configured repeated-identical/no-progress threshold, and one concise receipt. Follow up, repair, replan, and independently re-review while progress changes. Add prompt-quality as a fourth read-only brief only when the scoped diff changes prompts or agent instructions. Consolidation is forbidden until each required dimension has a receipt."
    ].join("\n");
  ...
  if (name === "runner.md") {
    ...
    const codexScheduling = "**Scheduling** — fire `$muster-runner` from a Codex automation on a timer, or from cron invoking the Codex CLI (e.g. `codex exec \"$muster-runner issues:agent:todo\"`). Codex has no equivalent recurrence primitive, so a fixed cadence is the only mechanism here: match the backlog's arrival rate, not faster — an idle cycle is cheap, but firing far ahead of new items just piles up idle receipts; start around 15-30 minutes and widen if idle cycles dominate the ledger, tighten if items queue up unclaimed. Safety inventory: pr-only (step 4 never merges/pushes to base), progress-aware repeated-outcome recovery bounds reclaim loops, and the claim lock (Binding A's comment-race window / Binding B's claim-then-verify) makes concurrent firing safe alongside `$muster-go-backlog`, other scheduled runners, and humans on the same backlog.";
