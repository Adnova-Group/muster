...
function adaptCommandForCodex(text, name, contract) {
  ...
  let result = translatePluginPaths(translateCodexProse(text))
    ...
    .replace(registryFallbackRe, "construct the complete wave and run it through `node ${PLUGIN_ROOT}/runtime/muster.mjs codex-wave <wave.json>` with trusted repository/base inputs supplied out of band. Production waves are process-only: never call `spawn_agent` directly from the backlog or manifest. The runtime executes every `muster-runner` member in its registered linked worktree and owns the bounded barrier; treat any missing provenance or process failure as an item failure.")
