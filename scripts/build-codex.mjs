...
function codexSkill(source, id, contract) {
  ...
  if (id === "review-gate") {
    ...
    body = body.replace(
      fixLoopPrefixRe,
      "6. If `blocked`: load the exact original implementer receipt written by `fix-loop-bind`. For a process-wave member, that receipt must bind the full resolved `muster-runner` profile. Write the current cwd, base SHA, Codex version, and full resolved role profile to a current-context file, and retained `sentBlockers` plus this review's `currentBlockers` to a review-state file. Run `node ${PLUGIN_ROOT}/runtime/muster.mjs fix-loop-continue <receipt.json> <current-context.json> <review-state.json>`; it validates the profile fingerprint and other bindings and derives only the new blocker deltas. Execute only its returned plan: `collaboration.followup_task` for the exact canonical in-session worker id, or its `codex exec resume <thread-id>` argv from the returned bound cwd for the exact process `thread.started` id. Never use a recency selector, never fresh re-dispatch, and never repeat the original brief or prior transcript. Then re-review. Cap at"
    );
