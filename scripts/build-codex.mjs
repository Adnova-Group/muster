...
function adaptCommandForCodex(text, name, contract) {
  ...
  if (name === "audit.md") {
    ...
    const capacitySweep = [
      ...
      "   Dispatch these three briefs concurrently when the configured Codex capacity permits, otherwise in dependency-free batches. Respect `agents.max_concurrent_threads_per_session`; neither lower nor raise it. Before each dispatch, inspect THIS active session's callable namespaces (`multi_agent_v1.spawn_agent` means v1; `collaboration.spawn_agent` means v2), then run `node ${PLUGIN_ROOT}/runtime/muster.mjs codex-audit-provider --role <role> --task-id <id> --callable-apis <v1,v2> --message-file <brief>`. The CLI derives the role's full manifest-ordered independent-agent chain, explicit availability from live inventory, each profile's model, and its API from the current Codex catalog; dispatch ONLY its returned packet. It preserves the preferred provider when compatible, skips stale incompatible/unavailable preferences for the first independent compatible provider, and emits `{mode:\"inline\", packet:null, degradation:{code:\"CODEX_AUDIT_NO_COMPATIBLE_PROVIDER\",...}}` only when none exists. Append that machine-readable degradation to STATE; never construct or send a packet for a namespace absent from this session. Every worker uses `fork_turns: \"none\"`, an explicit cohesive-task budget, focused commands, a deterministic progress fingerprint, a configured repeated-identical/no-progress threshold, and one concise receipt. Follow up, repair, replan, and independently re-review while progress changes. Add prompt-quality as a fourth read-only brief only when the scoped diff changes prompts or agent instructions. Consolidation is forbidden until each required dimension has a receipt."
    ].join("\n");
