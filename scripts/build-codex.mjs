...
function adaptOrchestratorForCodex(text) {
  ...
  result = result.slice(0, waveDispatchStart)
    + `${waveDispatchHeading}\n\nCodex has no counterpart to Claude Code CLI's agent-teams \`Workflow\` tool. Before dispatching each wave, write its member packet to \`.muster/codex-wave.json\` and run \`node ${"${PLUGIN_ROOT}"}/runtime/muster.mjs codex-wave .muster/codex-wave.json\`. The runtime reads \`agents.max_concurrent_threads_per_session\` from \`${"${CODEX_HOME:-$HOME/.codex}"}/config.toml\`, combines that configured ceiling with the packet's desired and currently available limits, and returns \`effectiveCeiling\`. Overlapping \`writes\` sets select the \`exec-process\` lane: the bundled runtime launches every member in its own required \`cwd\` without exceeding that ceiling, pins \`--ignore-user-config\`, \`--strict-config\`, \`--ephemeral\`, sandbox, and approval policy, rejects an unsupported Codex version or feature surface, parses \`turn.completed\` usage (or reports \`UNKNOWN\`), and propagates every nonzero exit. Disjoint writes select \`spawn_agent\`: process the returned \`batches\` in order, invoke one batch's exact per-model v1/v2 packets through \`multi_agent_v1\` (Luna/v1) or \`collaboration\` (Sol/Terra/v2), complete the matching wait/list barrier for every member in that batch, and only then dispatch the next batch. Never flatten batches or rewrite returned packets in prose. Only an explicit \`multiAgent: false\` (or \`MUSTER_CODEX_MULTI_AGENT=0\`) drops dispatch to \`mode: "sequential-inline"\` -- one crew member at a time, never a partial/mixed fan-out.\n\n### Worktree isolation: receipts-only\n\n\`collaboration.spawn_agent\` has no cwd field, so Codex has no native worktree mechanism to select at all: run \`node ${"${PLUGIN_ROOT}"}/runtime/muster.mjs worktree-isolation --harness codex\` to confirm this harness always resolves \`mechanism: "receipts-only"\`. The brief's absolute \`WORKTREE CWD\` (Provider and model policy, above) plus a base-SHA receipt per dispatched crew member -- \`{taskId, mechanism, baseSha, worktreePath}\`, refused over a missing or non-hex \`baseSha\` -- stand in for the isolation guarantee muster cannot get from this harness; append the receipt to STATE alongside the dispatch line. Immediately after, run \`node ${"${PLUGIN_ROOT}"}/runtime/muster.mjs receipt-verify <baseSha> --cwd <absolute worktree path>\` and treat a nonzero exit as a receipt failure -- escalate it, never continue silently.\n\n`
