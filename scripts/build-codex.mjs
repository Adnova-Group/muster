...
function adaptCommandForCodex(text, name, contract) {
  ...
  if (name === "audit.md") {
    ...
    const capacitySweep = [
      ...
      "   Dispatch these three briefs concurrently when the configured Codex capacity permits, otherwise in dependency-free batches. Respect `agents.max_concurrent_threads_per_session`; neither lower nor raise it. Before each dispatch, inspect THIS active session's callable namespaces (`multi_agent_v1.spawn_agent` means v1; `collaboration.spawn_agent` means v2), then resolve the manifest-ordered independent agent candidates' model API versions from the current Codex catalog and explicit availability from the live inventory. Pass that ordered matrix to `node ${PLUGIN_ROOT}/runtime/muster.mjs codex-audit-provider <request.json>` and dispatch ONLY its returned packet: it preserves the preferred provider when compatible, skips stale incompatible/unavailable preferences for the first independent compatible provider, and emits `{mode:\"inline\", packet:null, degradation:{code:\"CODEX_AUDIT_NO_COMPATIBLE_PROVIDER\",...}}` only when none exists. Append that machine-readable degradation to STATE; never construct or send a packet for a namespace absent from this session. Every worker has a 25-step ceiling, focused commands only, and one concise receipt. Add prompt-quality as a fourth read-only brief only when the scoped diff changes prompts or agent instructions. Consolidation is forbidden until each required dimension has a receipt."
    ].join("\n");
