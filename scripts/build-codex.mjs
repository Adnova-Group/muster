...
// other by hand. codex-agent-watch-review-budget item (2026-07-19 dogfood): the flat
// 3-heartbeat kill interrupted a healthy gpt-5.6-sol/high muster-reviewer mid-review; the
// watch is now liveness-aware (an in-turn worker is extended, not killed) with per-class
// ceilings bounding how far the extension goes.
  ...
  if (id === "review-gate") {
    body = body
      ...
      .replace(
        ...
        "6. If `blocked`: load the exact original implementer receipt written by `fix-loop-bind`. Write the current cwd, base SHA, Codex version, and full resolved role profile to a current-context file, and retained `sentBlockers` plus this review's `currentBlockers` to a review-state file. Run `node ${PLUGIN_ROOT}/runtime/muster.mjs fix-loop-continue <receipt.json> <current-context.json> <review-state.json>`; it validates the profile fingerprint and other bindings and derives only the new blocker deltas. Execute only its returned plan: `collaboration.followup_task` for the exact canonical in-session worker id, or its `codex exec resume <thread-id>` argv from the returned bound cwd for the exact process `thread.started` id. Never use a recency selector, never fresh re-dispatch, and never repeat the original brief or prior transcript. Then re-review. Allow **one fix-and-re-review iteration**. If the same blocker remains or continuation validation fails, ESCALATE"
      );
  }
