...
//
// CANONICAL SEAM, no direct production caller today: the in-repo code paths
// that actually read a backlog file -- src/scope.js's readBacklogCandidate
// (scope detection) and src/cli.js's sprint-waves branch (the orchestrator's
// real backlog read, audit 2 slice B) -- apply this SAME canonical
// containment check themselves via resolveContainedRealpath. This function
// exists as the single code-backed resolution step for the PROSE-side
// consumers: plan-backlog.md B1 and go-backlog.md step 1 treat
// parseBacklogRef's kind:"file" as a green light to read `path`, and any
// future code consumer of that classification must resolve through HERE (or
// replicate the check deliberately) rather than reading the un-resolved
// path. Keep it exported and tested (test/fs-safe.test.js) even while
// unwired -- deleting it would orphan the prose contract with no code seam
// to point at.
