...
async function handleKimiCommand(cmd, rest) {
  if (cmd === "kimi-goal-invocation") {
...
async function handleChatgptCommand(cmd, rest) {
  if (cmd !== "install" || rest[0] !== "chatgpt-work") return false;
...
async function handleCoreCommandPart1(cmd, rest) {
  if (cmd === "detect") {
...
async function handleCoreCommandPart2(cmd, rest) {
  if (cmd === "gate-cadence") {
...
async function handleCoreCommandPart6(cmd, rest) {
  if (cmd === "sprint-waves") {
    ...
    const assertSafeMutationPath = () => assertContainedNoSymlinkPath(runRoot, target, { allowMissingFinal: true });
    ...
    const result = await withFileMutationLock(target, async () => {
      ...
      const actual = prior === null ? "absent" : createHash("sha256").update(prior.bytes).digest("hex");
      if (actual !== expected) {
...
async function handleCoreCommandPart3(cmd, rest) {
  if (cmd === "fuse") {
...
async function handleCoreCommandPart7(cmd, rest) {
  if (cmd === "design") {
    const sub = rest[0];
    const target = flagValue(rest, "--target");
    const optionValues = new Set([
      flagValue(rest, "--content-file"), target, flagValue(rest, "--outcome"),
      flagValue(rest, "--add"), flagValue(rest, "--wave"), flagValue(rest, "--args"),
    ].filter(Boolean));
    ...
    if (sub === "workflows") {
      ...
      out(await initializeDesign(positionals[0] || process.cwd(), {
        target, contentFile: flagValue(rest, "--content-file"),
      }));
    } else if (sub === "status") {
      ...
      out(await scanDesign(positionals[0] || process.cwd(), { target, wave: flagValue(rest, "--wave") || "cli" }));
    } else if (sub === "ignores") {
      ...
      out(await designGate(positionals[0] || process.cwd(), {
        target, outcome, write: !rest.includes("--read-only"), audit: rest.includes("--audit"),
      }));
    } else if (sub === "run") {
      ...
      out(await runDesignWorkflow(positionals[1] || process.cwd(), workflow, {
        target, args: flagValue(rest, "--args") || "",
      }));
...
async function handleCoreCommandPart4(cmd, rest) {
  if (cmd === "route") {
...
async function handleCoreCommandPart5(cmd, rest) {
  if (cmd === "hygiene") {
...
async function handleCoreCommand(cmd, rest) {
  if (await handleCoreCommandPart1(cmd, rest)) return true;
  if (await handleCoreCommandPart2(cmd, rest)) return true;
  if (await handleCoreCommandPart6(cmd, rest)) return true;
  if (await handleCoreCommandPart3(cmd, rest)) return true;
  if (await handleCoreCommandPart7(cmd, rest)) return true;
  if (await handleCoreCommandPart4(cmd, rest)) return true;
  if (await handleCoreCommandPart5(cmd, rest)) return true;
  return false;
}
...
async function main() {
  ...
  try {
    ...
    if (await handleKimiCommand(cmd, rest)) return;
    if (await handleCodexCommand(cmd, rest)) return;
    if (await handleChatgptCommand(cmd, rest)) return;
    if (await handleCoreCommand(cmd, rest)) return;
