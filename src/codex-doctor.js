...
export async function runCodexDoctor({ root, cwd = process.cwd(), codexHome, execFile } = {}) {
  ...
  const plugin = await exists(join(base, ".codex-plugin", "plugin.json"))
    ? base
    : join(base, ".agents", "plugins", "plugins", "muster");
  const checks = [];
  ...
  try {
    const primary = join(base, "codex", "agents");
    const profileDir = await exists(primary) ? primary : join(plugin, "agents");
  ...
  for (const dir of hookHomes) {
    try {
      ...
      if (owner.owner === "muster" && configured && runtime.every(Boolean)) { hookStatus = dir; break; }
