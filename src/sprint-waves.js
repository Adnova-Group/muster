...
function validateSprintPlan(plan) {
  ...
  if (isRecord(schedule.integration)) {
    ...
    if (!sameIds(schedule.integration.dispositions, ["merge-local", "merge-push"])) errors.push("plan.schedule.integration.dispositions is invalid");
  }
  ...
  for (let waveIndex = 0; waveIndex < plan.waves.length; waveIndex += 1) {
    ...
    const expectedIntegration = expectedIds.filter((id) =>
      ["merge-local", "merge-push"].includes(plan.items[id]?.disposition));
    if (!sameIds(scheduled.integration.itemIds, expectedIntegration)) {
      errors.push(`plan.schedule.waves[${waveIndex}].integration.itemIds must match merge dispositions in backlog order`);
    }
  }
