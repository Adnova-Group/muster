...
test("Codex doctor verifies the bundled MCP initialize and tools/list handshake", async () => {
  ...
  assert.match(handshake?.detail || "", /31\/31.*Codex may defer MCP tool visibility until lookup or a new session/i);
...
test("Codex doctor reports MCP launch and tool-count handshake failures", async () => {
  ...
  for (const [label, mcpRunner, expected] of [
    ...
    ["tool-count", async () => ({ initialized: true, tools: Array.from({ length: CODEX_COUNTS.mcpTools - 1 }, () => ({})) }), /30\/31/]
  ]) {
    const report = await runCodexDoctor({
      root: repoRoot,
      cwd: repoRoot,
...
// readRegularFile's `fsSafe.reason === "changed" -> musterUnsafeRead
// ("Codex configuration target changed while reading")` translation
// (src/codex-doctor.js:436) was pinned only at the fs-safe primitive, never at
// this call site. A same-user writer replacing the target mid-read trips the
