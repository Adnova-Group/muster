...
// outcome's actual engineering vocabulary (validates/verifies/verification/SHA/TDD), so the
// direct keyword match misses -- and the workspace-shape fallback ALSO misses for a repo like
// muster's own (no package.json main/exports, no FE/BE framework deps -> shape stays
// "unknown", proven directly against detectProject(process.cwd()) in this checkout), so
// classifyDomain falls all the way through to domain:unknown, source:none. This is the exact
// dogfood fixture (also used verbatim in test/interview.test.js), classified with NO profile
// at all -- i.e. the fix must come from the outcome-text keyword grammar, not from the
// workspace-shape fallback.
...
test("dogfood fixture: concrete code-outcome vocabulary (validates/verifies/verification/SHA/TDD) classifies as software with NO profile fallback", () => {
...
test("engineering vocabulary: 'validates'/'verifies' alone routes to software", () => {
  assert.equal(classifyDomain("the loader validates the manifest shape before it verifies checksums").domain, "software");
});
...
// -- "verification" must not fire from an unrelated word merely containing "verify"/"sha" as a
// substring (e.g. "unverified", "shape", "washable").
