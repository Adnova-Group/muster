...
test("generateNotice preserves a pinned attribution-only source label", () => {
  const n = generateNotice([{
    provenance: {
      adapted_from: "pbakaus/impeccable @ 32930818a109fafa87199babe92fa8e530cff5d3",
      noticeSource: "pbakaus/impeccable @ 32930818a109fafa87199babe92fa8e530cff5d3",
      license: "Apache-2.0",
    },
  }]);
  assert.match(n, /pbakaus\/impeccable @ 32930818a109fafa87199babe92fa8e530cff5d3 \(Apache-2\.0\)/);
});
...
test("runVendor: rejects a symlinked source item that escapes the source root", async (t) => {
  const { home, repoRoot } = await tmpDirs();
  const outside = await mkdtemp(join(tmpdir(), "muster-vendor-source-victim-"));
  t.after(() => Promise.all([rm(home, { recursive: true, force: true }), rm(repoRoot, { recursive: true, force: true }), rm(outside, { recursive: true, force: true })]));
  const skills = join(home, SP_BASE, "v1.0.0", "skills");
  await mkdir(join(skills, "evil"), { recursive: true });
  await mkdir(join(repoRoot, "catalog"), { recursive: true });
  const victim = join(outside, "SKILL.md");
  await writeFile(victim, "---\nname: stolen\n---\n\nSECRET\n");
  try { await symlink(victim, join(skills, "evil", "SKILL.md")); }
  catch (error) { t.skip(`symlinks unavailable: ${error.code}`); return; }

  const manifest = { sources: [{ id: "superpowers", kind: "local", repo: "obra/superpowers", license: "MIT",
    items: [{ from: "evil/SKILL.md", id: "sp-evil", roles: ["brainstorm"] }] }] };
  const result = await runVendor({ home, repoRoot, manifest });
  assert.equal(result.count, 0);
  assert.ok(result.warnings.some((warning) => /unsafe source path/i.test(warning)));
  await assert.rejects(lstat(join(repoRoot, "plugin/builtins/sp-evil/SKILL.md")), { code: "ENOENT" });
});

test("runVendor: rejects symlinked output ancestry without publishing outside repoRoot", async (t) => {
  const { home, repoRoot } = await tmpDirs();
  const outside = await mkdtemp(join(tmpdir(), "muster-vendor-output-victim-"));
  t.after(() => Promise.all([rm(home, { recursive: true, force: true }), rm(repoRoot, { recursive: true, force: true }), rm(outside, { recursive: true, force: true })]));
  await seedSuperpowers(home, { "ok/SKILL.md": "---\nname: ok\n---\n\nSAFE\n" });
  await mkdir(join(repoRoot, "plugin"), { recursive: true });
  await mkdir(join(repoRoot, "catalog"), { recursive: true });
  try { await symlink(outside, join(repoRoot, "plugin", "builtins"), "dir"); }
  catch (error) { t.skip(`directory symlinks unavailable: ${error.code}`); return; }
  const manifest = { sources: [{ id: "superpowers", kind: "local", repo: "obra/superpowers", license: "MIT",
    items: [{ from: "ok/SKILL.md", id: "sp-ok", roles: ["brainstorm"] }] }] };

  await assert.rejects(runVendor({ home, repoRoot, manifest }), /unsafe output path/i);
  assert.deepEqual(await readdir(outside), []);
});

test("runVendor: publishes generated files atomically as regular files", async (t) => {
  const { home, repoRoot } = await tmpDirs();
  t.after(() => Promise.all([rm(home, { recursive: true, force: true }), rm(repoRoot, { recursive: true, force: true })]));
  await seedSuperpowers(home, { "ok/SKILL.md": "---\nname: ok\n---\n\nSAFE\n" });
  await mkdir(join(repoRoot, "catalog"), { recursive: true });
  const manifest = { sources: [{ id: "superpowers", kind: "local", repo: "obra/superpowers", license: "MIT",
    items: [{ from: "ok/SKILL.md", id: "sp-ok", roles: ["brainstorm"] }] }] };
  assert.equal((await runVendor({ home, repoRoot, manifest })).count, 1);

  for (const relative of ["plugin/builtins/sp-ok/SKILL.md", "catalog/builtins.generated.yaml", "NOTICE"]) {
    const published = join(repoRoot, relative);
    assert.equal((await lstat(published)).isFile(), true, `${relative} is a regular file`);
    assert.ok((await readFile(published)).length > 0, `${relative} has complete content`);
  }
  const names = await readdir(repoRoot, { recursive: true });
  assert.equal(names.some((name) => name.includes(".muster-tmp-")), false, "no publication temp survives");
});
