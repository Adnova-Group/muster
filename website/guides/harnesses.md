...
| --- | --- | --- | --- | --- | --- |
| Claude Code | Nine `/muster:*` commands | Native agents; optional Workflow fan-out | Native agent worktrees | Plugin hooks plus review gates | Native `/init`, canonical instruction pair |
| Codex | Nine `$muster-*` skills | `spawn_agent` or isolated `codex exec` processes | Muster-created worktrees and receipts | Advisory hooks plus review gates | Native `/init`, canonical instruction pair |
| Kimi | Nine `/muster-*` skills | In-session native subagents; attended process lane is report-only | Muster-created worktrees and base-SHA receipts | Native permission rules plus review gates | Unavailable; explicit acknowledgement required |
...
| ChatGPT Work | Universal plugin + registered MCP connection | Pro-safe `muster_prioritize` only after native Scan Tools; full profile requires entitlement + double opt-in | Host-controlled; no Muster worktree claim | ChatGPT approvals/admin controls plus native proof gate | No callable Init adapter; no Codex-config inheritance |
