...
**`PreToolUse`** (`pre-tool-use.js`) enforces the wave-guard iron rule. While `.muster/wave-active` exists, any Edit, Write, NotebookEdit, or high-confidence Bash file write originating from the orchestrator main loop (not from a crew subagent) is blocked. Decision order: (1) subagent calls always allowed, (2) writes into `.muster/` or `.claude/` always allowed (state bookkeeping and repo-local settings), (2.6) the action-class fence (below, independent of wave state), (3) if no wave marker exists, apply the per-turn scale gate (may deny), (4) same when `.muster/run-active` is absent while a wave-active marker exists -- primary crashed-wave signal (verbs write `run-active` at invocation start and clear it at end; `SessionStart` clears stale); or when the wave-active marker is older than 60 minutes (fallback stale detection), (5) honour `MUSTER_WAVE_GUARD`: `off` = silent allow, `warn` = allow with a reminder, unset or `deny` = deny, and any non-`Edit`/`Write`/`NotebookEdit` tool call falls through allowed. For Bash the guard is deliberately conservative (fail-open): only `sed -i`, `tee` to a non-exempt target, and `>` / `>>` redirects to non-exempt targets are denied. Exempt targets: `/dev/*`, `/tmp/*`, `.muster/*`.

The same hook also applies a **post-run scale gate** once a run completes and `.muster/wave-active` is gone. Main-loop inline work that crosses the 3-file boundary (`MUSTER_INLINE_SCALE`, default 3) in a turn with no active wave is denied and routed to a verb. The gate counts both editor tools (Edit, Write, NotebookEdit) and high-confidence Bash file writes toward the per-turn limit. This closes the drift gap that SessionStart guidance and the UserPromptSubmit nudge could not hold in the post-run window. A cumulative counter tracks the same distinct files across turns (not just within one), warning once per session -- never denying -- when the running total reaches the same threshold with no run active; it survives `/compact` and resume, and resets at `SessionStart` and while a run is active.

Independent of wave state, the same hook also enforces an **action-class fence** (`action-guard.js`): when both `.muster/run-active` and `.muster/forbidden-actions` exist, it classifies the tool call (an `mcp__`-prefixed tool name against a `send`/`submit`/`publish`/`sign`/`purchase` keyword set, word-boundary matched; or a Bash command against a small high-confidence allowlist -- `git push`, `npm publish`, `gh release create`, `gh pr merge`, `curl -X POST`) and denies a match against the run's declared `forbiddenActions`, honouring `MUSTER_ACTION_GUARD` (`off`/`warn`/deny-by-default). The `hooks.json` matcher was widened to also catch send/submit/publish/sign/purchase-shaped tool names so this classification runs; either file absent, or no class matching, is a no-op.

**`PreToolUse` on `Task`/`Agent`** (`todo-gate.js`) enforces the **todo-driving gate**. During a live Muster run (`.muster/run-active` present), dispatching a subagent wave via the `Task` or `Agent` tool is denied unless a native todo list was written since the run started, so plan progress stays visible in Claude Code's todo UI -- current Claude Code renamed the dispatch tool from `Task` to `Agent`, so the gate matches both names (it was silently inert on the old `Task`-only match). The hook tail-reads the session transcript for a `TodoWrite`/`TaskCreate`/`TaskUpdate` call with a timestamp at or after the `run-active` mtime (minus a 2s grace margin). It biases hard toward allow: a non-dispatch tool, no live run, `MUSTER_TODO_GATE=off`, or any unreadable transcript state all allow (`warn` allows with a note); only a readable transcript with provably no qualifying todo write denies.
