...
| `codex-wave-resume <receipt-id> --review-state <file>` | Authenticate an opaque process-wave receipt from the owner-only runtime store, independently re-derive executable/version, repository/worktree/base/HEAD, runner policy, and action fence, then resume the exact persisted thread inside the original bubblewrap and hermetic limits. New reviewer findings are bounded, labeled as untrusted data, and sent over stdin; callers never receive a raw host-side resume command. |
