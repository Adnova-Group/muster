...
| `fix-loop-bind <dispatch.json> <receipt.json>` | Persist a Codex implementer's canonical worker/thread id, cwd, base SHA, Codex version, and SHA-256 fingerprint of the full resolved role profile. The receipt is create-only so a later dispatch cannot silently replace its identity. |
