...
| `codex-wave <wave.json> --repository-root <repo> --base-sha <sha>` | Hermetic Codex wave execution and the sole production-wave route. Every member declares `agentType: "muster-runner"` and runs as a separately bounded `codex exec -C` process in a registered pristine linked worktree. The runtime loads the installed runner instructions and pins their digest, model, effort, sandbox, trusted Codex executable, repository root, and exact base SHA outside the manifest. |
