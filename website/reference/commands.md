...
| `fix-loop-bind <dispatch.json> <receipt.json>` | Persist a Codex implementer's canonical worker/thread id, cwd, base SHA, Codex version, and SHA-256 fingerprint of the full resolved role profile. The receipt is create-only and must remain inside the bound worktree, so a later dispatch cannot silently replace its identity or redirect publication through a symlinked path. |
| `fix-loop-continue <receipt.json> <current.json> <review-state.json>` | Validate the retained receipt against current isolation state, derive blocker findings not already sent, and emit an exact-ID `followup_task` or `codex exec resume <thread-id>` plan. Context drift, an empty blocker delta, and recency-based resume all fail loud. |
