...
| `codex-wave <wave.json>` | Live Codex wave selector. A wave with overlapping `writes` sets runs members concurrently as isolated `codex exec -C <cwd>` processes with hermetic config, sandbox, approval, version/feature, JSONL usage, and exit-code enforcement. A disjoint wave emits the exact per-model v1/v2 collaboration packets for the running orchestrator to invoke. |
