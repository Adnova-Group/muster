...
| `advise <advice-request.json>` | Validate an advice request and emit the structured advisor dispatch input (`advisorModel` + `request`). Deterministic, no LLM calls. The advisor role resolves to apex, degrading to prime when apex is disabled. |
...
| `muster_advise` | `advise` | Validate an advice request and resolve the advisor model (apex to prime fallback). |
