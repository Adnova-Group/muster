...
| `codex-wave <wave.json> --repository-root <repo> --base-sha <sha>` | Hermetic Codex wave execution. Every production wave is process-only: each member runs as a separately bounded `codex exec -C` process in a registered linked worktree because manifest write fences are not mechanically enforceable in shared-CWD subagents. The trusted repository root and exact full base SHA are mandatory and validated before lane resolution. The manifest cannot override `CODEX_HOME` or model-catalog versions. |
