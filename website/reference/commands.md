...
| `backlog-publish <backlog.md> --expect <sha256\|absent>` | Compare-and-swap publish complete backlog content read from stdin. The relative path must remain contained and symlink-free. A stale expected digest fails closed; reread, reapply, and retry rather than overwriting a concurrent writer. |
...
| `muster_backlog_publish` | `backlog-publish` | Compare-and-swap publish a complete backlog under an explicit project root. The contained, symlink-free relative path and expected prior SHA-256 prevent silent concurrent-writer loss. |
