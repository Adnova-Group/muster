...
| `backlog-receipts <backlog.md> --release-ref <ref>` | Rejects every checked, non-withdrawn file-backlog item unless its single `{merge: <SHA>}` or `{done: <SHA>}` receipt is a full lowercase commit SHA reachable from the declared release ref. `{withdrawn: <reason>}` is the explicit exemption. Exits `0` only when all checked items pass, `2` with every rejected item otherwise. |
