...
| `kimi-process-dispatch --brief <text> --agent-file <name\|path> --cwd <dir> --lane <primary\|secondary>` | Print the descriptor (`{argv, env, cwd, lane}`) for one headless `kimi -p <brief> --agent-file <path>` process — the attended-session process lane for lane-sensitive legs (`src/kimi-dispatch.js`'s `kimiProcessDispatch`). `-m` is always emitted (omitting it silently falls to config `default_model`); `env` is the same OVERRIDE pair, merged over the ambient env at spawn. A bare agent-file name resolves under the installed Kimi agents dir; an explicit path must exist. See `plugin/skills/orchestrator/references/kimi-dispatch.md`. |
...
| `hygiene [--reap] [--json] [--backlog <file>] [--worktree-threshold N] [--zombie-stale-min N] [--claim-stale-min N]` | Burn-hygiene guards so a dead run can't strand machine state: detects zombie provider CLI (codex/claude) processes (an orphaned process -- parent dead/1 -- whose Muster provenance is corroborated, via a cwd under a known Muster run worktree or a dispatch receipt, is reap-eligible; an orphan with no Muster provenance and a merely stale-started process with a live parent are report-only, never killed), offers a stale-worktree sweep once live worktrees exceed the threshold (report-only, never deletes), and auto-releases a backlog `{claimed: runner@ts}` annotation once its heartbeat exceeds the stale-claim threshold (default 60 min). Report-only by default; `--reap` opts into killing reap-eligible zombies and rewriting the backlog to release stale claims. |
...
| `muster_advise` | `advise` | Validate an advice request and resolve the advisor model (apex → prime; Claude-concrete value emitted beside it). |
