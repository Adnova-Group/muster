...
Muster names a fixed vocabulary of **roles**, the kinds of work a crew might need (`implement`, `code-review`, `test-author`, `debug`, `security-review`, `author`, `research`, `score`, `humanize`, and more). Roles are the stable interface: pipelines and commands ask for a role, not for a specific tool.
