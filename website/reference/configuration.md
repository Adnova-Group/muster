...
| `MUSTER_MCP_HOST` | _(adapter-set)_ | Internal host selector set by `mcp/codex-server.mjs`, `mcp/chatgpt-work-server.mjs`, or the Cowork compatibility shim. Do not set it manually; it selects the neutral core's host protocol and tool profile. | `mcp/server.mjs` |
...
| `MUSTER_COWORK_MAX_INFLIGHT` | `4` | Max concurrent MCP tool executions in the Cowork server. Hard ceiling `64`. | `mcp/server.mjs` (via `cowork/mcp-server.mjs` compatibility shim) |
| `MUSTER_COWORK_MAX_QUEUE` | `16` | Max queued MCP tool executions before overload rejection. Hard ceiling `1024`. | `mcp/server.mjs` (via `cowork/mcp-server.mjs` compatibility shim) |
