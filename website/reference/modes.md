...
If either file existed at the preparation baseline and prevents that exact authority pattern, Init does not overwrite or merge it. It leaves a HUMAN-HOLD for the user to reconcile. A partial pair, independent policy in `CLAUDE.md`, or a reverse `AGENTS.md` reference to `CLAUDE.md` cannot satisfy artifact-delta, pre-existing-confirmation, or call-result evidence.
